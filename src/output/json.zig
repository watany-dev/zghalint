...
/// Bytes that a JSON string must escape: `"` `\` and C0 controls.
const json_special: []const u8 = "\"\\\x00\x01\x02\x03\x04\x05\x06\x07\x08\t\n\x0b\x0c\r\x0e\x0f" ++
    "\x10\x11\x12\x13\x14\x15\x16\x17\x18\x19\x1a\x1b\x1c\x1d\x1e\x1f";

fn writeJsonChars(writer: *std.Io.Writer, s: []const u8) !void {
    var i: usize = 0;
    while (i < s.len) {
        const idx = std.mem.findAnyPos(u8, s, i, json_special) orelse {
            try writer.writeAll(s[i..]);
            return;
        };
        try writer.writeAll(s[i..idx]);
        try writeJsonEscape(writer, s[idx]);
        i = idx + 1;
    }
}

fn writeJsonEscape(writer: *std.Io.Writer, c: u8) !void {
