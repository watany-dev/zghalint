# 実施ロードマップ（2026-09-10 時点）

オープンな PR / issue を main（`d43a3f4`）の実装状況と突き合わせ、以後の実施順序を示す。
...
| `src/**/*.zig` | 60,899 行 |
| ユニットテスト | 2055 件（`zig build test` 緑）。#331 で低品質なテストの削除とテーブル駆動化を行った上で（PR #332）、SC007・ドッグフーディング・形式手法・autofix 拡張・fix の衝突修正・SEC023・SYN020・EXPR007 / EXPR011 の誤検出修正・性能最適化・空セクションのパース継続・SC003 のピンコメント版判定・BP007 のプロセス置換・SEC023 の crates.io・EXPR011 のオブジェクト軸・SYN021 / SYN022・BP003 の最新 major 比較・SEC016 の暗黙キャッシュ判定の分が積み増している。Zig 0.16 移行では stdout / stderr のリダイレクト保持と atomic replace の権限復元に回帰テストが加わった |
...
| ファズ | `src/fuzz_test.zig` が YAML パーサと式パーサのターゲットを持ち、CI で回る（#241）。長時間キャンペーン用に `src/fuzz_driver.zig` が単体ドライバとして立ち、PR #371 で入った。探索実行（`zig build fuzz --fuzz`）は Zig 0.16.0 で起動する。0.15.2 で出ていたファザ起動直後の失敗は再現しない（`docs/design/pbt-strategy.md` §6-4）。コールバックは `*std.testing.Smith` を受け取り、`slice` で最大 64 KiB の入力を作る。この経路が #364 / #366〜#370 の 6 件を掘り出して 6 件とも解決し（#366 は PR #376、#367〜#370 は PR #380、#364 は PR #390）、続く追加キャンペーンが #399 を出した。PR #380 は E2E ハーネスに全フィクスチャ共通の不変条件を 2 つ足しており、診断の span が逆転しないことと `--fix` / `--fix-unsafe` が不動点に達することを常設で見る |
| ADR | `docs/adr/0001`〜`0015`（0012 は RUNNER002 matrix 展開、0013 は RUNNER003、0014 は SEC023 trusted publishing、0015 は BP003 の最新 major 比較） |
| オープン PR | #217（形式仕様とモデル検査）、#306（bench 再実行記録）、#395（本ロードマップの同期）の 3 本。#392（#375 = D7）/ #393（#382 = G30）/ #394（#281 = G10、SYN021 / SYN022 追加）/ #396（#358 = G26 の BP003 最新 major 比較）/ #397（SEC016 の暗黙キャッシュと PERF001 の逆向き助言の抑制）/ #400（Zig 0.16 移行と `std.Io` 採用）/ #401（0.0.1-rc.2 へのバージョン更新）はマージ済みである。本ロードマップの #207 は `3951cc7` でマージ済みで、以後の同期はこのブランチが引き継ぐ。#362（#284 = G13）/ #363（#359 / #360 = G27 / G28）/ #365（性能最適化）/ #371（ファズドライバと #364〜#370 の起票）/ #373（#270 = bench 運用ループ）/ #374（install.sh と Homebrew tap）/ #376（#366 = taint テーブル溢れ）/ #377（perf bench に ghalint / octoscan / poutine / action-validator を追加）/ #379（SEC013 の GHCR login と BP007 の PowerShell 代入の FP、G29 の起票）/ #385（G30〜G32 の起票）/ #387（G33 の起票）/ #380（#367〜#370 の span 逆転と fix 非収束）/ #389（G31〜G33 = #383 #384 #386）/ #390（#364 の空セクション）/ #391（D6 = #372 の SC003 / SC005 / Release / skip 注記）が新たにマージされた。#306 は別セッションの PR のため本 PR からは触らない |
| オープン issue | 3 件。#399（autofix が再パース不能な YAML を作る）、#402（API 到達不能時のオンライン実行が 6.2 秒から 12.4 秒へ倍増）、#403（taint 追跡のホットパスで 0.16 が約 190 万命令の memset を出す）である。後 2 件は Zig 0.16 移行で顕在化したもので、いずれも `d43a3f4` に対する実測が issue に付いている。#398（Zig 0.16.0 への移行）は PR #400 が解決して close した。ドッグフーディング D7（#375）は PR #392 が、bench 由来の parity gap 3（#281 = G10 は PR #394、#358 = G26 は PR #396、#382 = G30 は PR #393）はそれぞれの PR が解決した。ファズ由来の 6 件は #366 が PR #376、#367〜#370 が PR #380、#364 が PR #390 で解決した。G30〜G33 のうち #383 / #384 / #386 は PR #389 で、D6（#372）は PR #391 で解決し、いずれも実バイナリでの再現確認を添えて close 済みである。追跡対象として残るのは issue を持たない SEC016 の暗黙キャッシュ FN だけである |
...
| 性能 | PR #365 が ReleaseFast のプロファイルからホットパスを削った——SEC002 の `${{` / `}}` 探索をベクトル化した `indexOfScalarPos` に、`pathMatchesPattern` を先頭バイトでの事前棄却つきに、taint 伝播の固定点を outputs を持つジョブだけに、SEC003 の秘密プレフィックス探索を 1 走査に、`didYouMean` を有界 Levenshtein の早期打ち切りに、cron の `nextAfter` を分単位から境界スキップに、terminal の `writeSanitized` を 16 バイト単位の読み飛ばしに、CA バンドル読込を最初の fetch まで遅延させた。`zig build -Dstrip` で Release でもシンボルを残せる。Zig 0.16 では `parseContextPath` と `walkJobTaint` が `= undefined` の固定長配列を持つ構造体を `.{}` で初期化する箇所で memset に落ち、callgrind の self Ir が 29 万から 193 万へ増えた（#403）。総命令数自体は 8040 万から 7880 万へ減っている |
| バージョン定義 | Zig の版は `build.zig.zon` の `minimum_zig_version` 一箇所が真で、現在は 0.16.0。リリース版は 0.0.1-rc.2。参照側の一覧と更新手順は `docs/maintenance.md`（#236） |
| 既知バグ | #399 が未修正である。パーサが受け入れた不正な YAML に autofix を当てると再パース不能な出力になり、`zig build fuzz-driver -- --file <case>` が 3 つの最小入力（29 / 19 / 25 バイト）で `FixBrokeParse` を返す。Zig 0.16 移行の回帰ではなく、`866a68e` を 0.15.2 でビルドしたバイナリでも同じく再現する。ネットワーク側では #402 が未修正で、`api.github.com` に届かない環境のオンライン実行が 0.15.2 の 6.2 秒から 0.16.0 で 12.4 秒に倍増する。std の接続プールが死んだ接続を張り直すようになり、`engine.setNetworkDeadline` の 10 秒がリクエスト間でしか見られないため総時間を縛れていない。ファズ由来の先行 6 件（#364 / #366〜#370）は全て解決済みである。クラッシュ（#366）も `--fix` がワークフローを壊す 3 件（#368〜#370）も span 逆転（#367）も空セクションでの lint 不能（#364）も潰れており、形式検証由来の反例と PR #261 で直した 7 件も同様である |
...
| 順 | 対象 | 理由 |
|---|---|---|
| 1 | #399（autofix が壊れた YAML を出す） | `--fix` の正しさそのものに関わる。パーサが受け入れた入力に対して再パース不能な出力を作るため、利用者のワークフローを壊しうる。最小入力 3 本が確定しているので、そのまま回帰テストにできる |
| 2 | #402（API 到達不能時の実行時間が倍増） | 利用者が体感する退行で、オフラインではない環境でネットワークが死んでいると 10 秒の期限を超えて 12.4 秒かかる。`prefetch.zig` に転送エラーの短絡を入れる案が小さく、リクエスト単位の期限を残り予算から導く案が期限の意味を回復させる |
| 3 | #403（taint 追跡の memset） | ホットパスの自己コストで、`ContextPath` と `TaintedNames` の宣言側で配列を明示的に `undefined` にするか、`walkJobTaint` の 1 KiB 構造体コピーをスライス渡しに変えるかで消える |
| 4 | #217 / #306 | 残る 2 本のドキュメント PR。#217 は形式仕様とモデル検査の反例、#306 は bench 全モードの再実行記録。#306 は別セッションの PR のため本 PR からは触らない |
...
移行後の実測から #402 と #403 の 2 件が新たに上がっている。

parity doc（`docs/design/external-linter-parity.md`）は G1〜G35 まで記録が追いつき、
...
D7（#375）を PR #392 で閉じ、追跡中のものは残っていない。
...
**6 件は PR #376 / #380 / #390 で全て片付き、続く追加キャンペーンが #399 を出した。**
