...
| 順 | 対象 | 理由 |
|---|---|---|
| 1 | #435〜#438（GA8〜GA11） | GA トラック第 3 段階の残り。GA7 が `cache-mode` を 2 能力へ解決する resolver を入れたので、setup-node の自動キャッシュ・checkout の credential・node20 の deprecated 化・concurrency の矛盾も同じ capability の形に載る。#435 は PR #517、#437 は PR #518 がレビュー待ちである |
...
| 3 | #439〜#441（GA12〜GA14） | GA トラック第 4 段階。merge key の非対応検出・`prefer-self-repository`・仕様追従用フィクスチャカタログで、追従漏れ自体を検出する側の整備である |
...
第 3 段階のうち GA7（#434）は PR #448 で着地し、`cache-mode` が `can_restore` / `can_save` の 2 能力へ解決され、SEC016 と PERF001 が同じ resolver を共有し、SEC024 が加わった。残る #435〜#438 は action 側の capability を同じ形で持ち、そのうち #435 / #437 は PR #517 / #518 が上がっている。
