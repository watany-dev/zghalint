...
オープンな PR / issue を main（`609a7bc`）の実装状況と突き合わせ、以後の実施順序を示す。
...
| 2 | #284 | 中身のないワークフローを `InvalidValue` ではなく診断として返すもので、bench に残る唯一の実行エラー。同じ「ファイルが丸ごと検査から落ちる」系の #349 は PR #357 が出ているため、残るのはこちらだけ |
...
実コーパスの FP（#305）は PR #317 で、ドッグフーディングの 5 本（#333〜#337）は PR #340〜#342 / #351 / #352 で全て片付いた。検出そのものの課題として残るのは、ファイルが丸ごと検査から落ちる #284（空ファイル。#349 は PR #357 待ち）とルール追加 2 本（#281 / #286）、そして実運用から新たに出た G26〜G28（#358〜#360）である。
