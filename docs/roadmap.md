...
| 2 | #424 / #425（G38 / G39） | どちらも 1 条件の緩和で閉じる FP。`parseContextAccess` がブラケット内を文字列リテラルに限っている分を数値まで広げ、`actionProblem` がローカル参照中の `@` を位置に関係なく ref とみなす分をセグメント先頭だけ除外する。既存の受理範囲を狭めない |
| 3 | #419（G36） | SEC002 が `startsWith` / `endsWith` / `contains` の真偽値だけを `run:` へ展開する形にも発火する FP。同じ切り分けは SEC006 が `if:` に対して既に持っており、`containsAnyContext` の呼び出し側を絞れば閉じる。パーサに触らない |
| 4 | #410 / #411（AF7 / AF8） | autofix 第 2 波の P1 のうち、既存基盤だけで閉じる 2 本。EXPR002 / EXPR003 / EXPR004 は `rename.tokenFix` が持つ did-you-mean 候補をそのまま safe fix にでき、ACT001 は composite の `run:` へ `shell:` を挿す `insertMappingEntry` で足りる |
...
過剰設計監査（#449 = PT0）の残る 9 件は振る舞いを変えない整理で、検出バグや GA トラックとは独立に進められる。うち 4 件は PR #500〜#503 が審査中のため、未着手は PT25 / PT27 / PT28 / PT29 / PT44 の 5 件である。
