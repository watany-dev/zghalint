...
オープンな PR / issue を main（`5caabb8`）の実装状況と突き合わせ、以後の実施順序を示す。
...
| `src/**/*.zig` | 51,822 行 |
| ユニットテスト | 1769 件（`zig build test` 緑） |
...
| E2E テスト | `src/e2e_test.zig` が `tests/fixtures/e2e/*.yml`（48 本）と `tests/fixtures/e2e-action/*.yml`（7 本）の `# zghalint:expect RULE@line` / `forbid` コメントを読んで検証 |
| bench | `bench/cases/` に 132 ファイル（複数ファイルのケースをまとめて 127 ケース）。A〜D は PR #277、E〜J は PR #287 で揃い、G16 / G17 の堅牢性ケース 3 本が PR #301 で加わった。`scripts/bench.py` の採点で zghalint は recall 100%（108/108）・precision 100%・位置一致 96%（104/108）。FN・FP ともに 0 件で、実行エラーは #284 の 1 件のみ。外部ツール未導入の環境では zghalint 単独で採点する。`--perf`（`scripts/bench_perf.py`）が wall time と最大 RSS を 4 シナリオで測り、実ワークフローのコーパスは `scripts/fetch-corpus.py` が取得する（#268、PR #296） |
...
| オープン PR | #207（本ロードマップ）、#217（形式仕様とモデル検査）、#306（bench 再実行記録）の 3 本。#272 / #277 / #287（bench）と #278（G5〜G8）/ #279（#159）/ #288（G11）/ #289（G12）/ #290（#64）/ #292（G9）/ #291（G14）/ #295（#294）/ #296（#268）/ #301（G16〜G19 の起票）/ #302（#293 = G16 / G17）/ #303（G18 / G19）はマージ済み。#306 は全ベンチモードの再実行記録と G20 / G21 の起票で、別セッションの PR のため本 PR からは触らない |
| オープン issue | 11 件。内訳はその他 1（#135）、形式手法 2（#307 の追跡 issue と #308 = F1）、ベンチマーク系 3（#262 本体と #269 / #270）、bench 由来の parity gap 5（#281 #284 #286 #304 #305 = G10 / G13 / G15 / G20 / G21）。#55 #64 #263 #264 #265〜#267 #268 #271 #273〜#276 #280 #282 #283 #285 #293 #294 #297〜#300 #159 は実装済みのため close 済み |
...
| bench 由来の parity gap #280〜#286 / #297〜#300 / #304 #305 | bench と実コーパスで出た FP・FN・実行エラーを `docs/design/external-linter-parity.md` の G9〜G21 として起票したもの。残るのは 5 本。ルール追加が #281（`needs:` の未知 job と循環）/ #284（中身のないワークフローを診断で返す）/ #286（API トークンでの publish を trusted publishing へ誘導）、実コーパス再実行で出た新規が #304（G20、ネットワーク不達時に SC003〜SC006 が黙って沈黙する）/ #305（G21、`actions/checkout` の `path:` が実行時に作るディレクトリへの `uses: ./` を DEP004 が誤検出）。パーサ層の #293 = #297 / #298（G16 / G17、親キーと同一桁のシーケンスと `-` 単独行）は PR #302、BP001 の reusable workflow 誤検出（G18 = #299）と fix エンジンの二重挿入（G19 = #300）は PR #303 で解消した。ルール改善の #285（G14、PERM001 の過剰警告）は PR #291、パーサ層の #280（G9、関数呼び出し結果へのプロパティ・インデックスアクセス）は PR #292、ファイル全体が検査対象から落ちる系の #282（G11、UTF-8 BOM）は PR #288、#283（G12、ドキュメントマーカー）は PR #289 で解消し、A〜D 由来の G5〜G8（#273〜#276）は PR #278 で解消済み |
...
| 2 | #305 | `actions/checkout` の `path:` が実行時に作るディレクトリへの `uses: ./...` を DEP004 が誤検出する（実コーパスで 44 件）。実利用でまとまった量の FP を出しているので、誤検出ゼロの方針から先に潰す |
| 3 | #284 / #304 | #284 は中身のないワークフロー（コメントのみのファイル）を `InvalidValue` ではなく診断として返す。bench に残る唯一の実行エラー `i-robustness/comments-only.yml` がこれで消える。#304 はネットワーク不達を「指摘なし」と区別できるようにする話で、どちらも出力の意味づけの問題 |
...
採点は全カテゴリを覆って recall 100% / precision 100%、G16 / G17 の修正で実コーパス 228 ファイルの
パース失敗も 36 件から 0 件になった。意図して用意したケースと実ワークフローの双方が通るようになったので、
次の課題は検出の有無ではなく出力の質——実コーパスで出る FP（#305）と、
沈黙と「指摘なし」の区別（#304）へ移っている。
