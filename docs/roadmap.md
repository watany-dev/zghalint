# 実施ロードマップ（2026-09-12 時点）

オープンな PR / issue を main（`a12d89a`）の実装状況と突き合わせ、以後の実施順序を示す。
...
| 項目 | 状態 |
|---|---|
| ルール数 | `docs/rules.md` の表は 107 行（SYN023 の `cache-mode` 値検証、SYN024 の `wait` 参照先 step id 検証、EXPR019 の未同期 background 出力参照、SEC024 の低信頼トリガー上の `cache-mode: write` を含む）で `registry.all_rules` と一致。`src/docs_sync_test.zig`（#242）が ID の欠落・余剰を両方向でテストする。同期テストは ID 集合しか見ないため見出しの数字は手で守る必要があり、現在の見出しは 107 で表と揃っている
| `src/**/*.zig` | 81 ファイル 66,627 行 |
| ユニットテスト | 2236 件（`zig build test` 緑）。#331 で低品質なテストの削除とテーブル駆動化を行った上で（PR #332）、SC007・ドッグフーディング・形式手法・autofix 拡張・fix の衝突修正・SEC023・SYN020・EXPR007 / EXPR011 の誤検出修正・性能最適化・空セクションのパース継続・SC003 のピンコメント版判定・BP007 のプロセス置換・SEC023 の crates.io・EXPR011 のオブジェクト軸・SYN021 / SYN022・BP003 の最新 major 比較・SEC016 の暗黙キャッシュ判定の分が積み増している。Zig 0.16 移行では stdout / stderr のリダイレクト保持と atomic replace の権限復元に回帰テストが加わり、PR #404 / #405 のファズキャンペーンがパーサとスパンと fix エンジンに 77 件を積み増した。GA トラック第 1 段階の PR #442〜#445 が SYN023・EXPR005 の偶数引数・RUNNER001 の macos-13 retired 化の分を、第 2 段階の PR #446 / #447 が `background` / `wait` / `parallel` の AST と SYN024・EXPR019 の分を足している。過剰設計監査の PR #498 は重複の集約で 602 行を落としながらテストを 4 件増やしており、`src/test_support.zig` へ寄せたルールテストの補助関数と `src/rules/ref_cache.zig` / `src/rules/expr_check.zig` の共通ヘルパがその受け皿である。第 3 段階の PR #448 が `cache-mode` の能力モデルと SEC024 の分を積み増した。過剰設計監査の PR #500〜#506 / #509 / #510 はネットワーク層と `glob.zig` と公開 API の整理で、振る舞いを変えないため件数は動かない。autofix 第 2 波の P1 4 本（PR #511〜#514）と検出バグ 4 本（PR #507 / #508 / #515 / #516）の分がここに積み増している |
...
| E2E テスト | `src/e2e_test.zig` が `tests/fixtures/e2e/*.yml`（108 本）と `tests/fixtures/e2e-action/*.yml`（9 本）の `# zghalint:expect RULE@line` / `forbid` コメントを読んで検証。兄弟ファイル `<name>.fixed` / `<name>.fixed-unsafe` を置いたフィクスチャは `--fix` / `--fix-unsafe` 適用後のバイト列も固定する（PR #328、現在 `tests/fixtures/e2e/` に 29 本、`tests/fixtures/e2e-action/` に 5 本） |
...
| 順 | 対象 | 理由 |
|---|---|---|
| 1 | #435〜#438（GA8〜GA11） | GA トラック第 3 段階の残り。GA7 が `cache-mode` を 2 能力へ解決する resolver を入れたので、setup-node の自動キャッシュ・checkout の credential・node20 の deprecated 化・concurrency の矛盾も同じ capability の形に載る。4 件とも PR がレビュー待ちである（#435 = PR #517、#436 = PR #523、#437 = PR #518、#438 = PR #519） |
| 2 | #414〜#417（AF11〜AF14） | autofix 第 2 波の残り。`secrets: inherit` の展開と SC003 の `patched_version` bump は既存基盤で届くが、SEC002 の github-script `script:` と SC001 のコンテナ digest ピンは JS 書き換えとレジストリ I/O のため別 ADR を要する |
| 3 | #439〜#441（GA12〜GA14） | GA トラック第 4 段階。merge key の非対応検出・`prefer-self-repository`・仕様追従用フィクスチャカタログで、追従漏れ自体を検出する側の整備である。3 件とも PR がレビュー待ちである（#439 = PR #520、#440 = PR #521、#441 = PR #522） |
...
第 3 段階のうち GA7（#434）は PR #448 で着地し、`cache-mode` が `can_restore` / `can_save` の 2 能力へ解決され、SEC016 と PERF001 が同じ resolver を共有し、SEC024 が加わった。残る #435〜#438 は action 側の capability を同じ形で持ち、4 件とも PR #517 / #523 / #518 / #519 が上がっている。
第 4 段階（#439〜#441）は merge key の非対応検出・`prefer-self-repository`・仕様追従用フィクスチャカタログで、追従漏れ自体を検出する側の整備であり、3 件とも PR #520 / #521 / #522 が上がっている。
