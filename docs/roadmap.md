...
オープンな PR / issue を main（`94fd1c9`）の実装状況と突き合わせ、以後の実施順序を示す。
...
| `src/**/*.zig` | 63,728 行 |
| ユニットテスト | 2132 件（`zig build test` 緑）。#331 で低品質なテストの削除とテーブル駆動化を行った上で（PR #332）、SC007・ドッグフーディング・形式手法・autofix 拡張・fix の衝突修正・SEC023・SYN020・EXPR007 / EXPR011 の誤検出修正・性能最適化・空セクションのパース継続・SC003 のピンコメント版判定・BP007 のプロセス置換・SEC023 の crates.io・EXPR011 のオブジェクト軸・SYN021 / SYN022・BP003 の最新 major 比較・SEC016 の暗黙キャッシュ判定の分が積み増している。Zig 0.16 移行では stdout / stderr のリダイレクト保持と atomic replace の権限復元に回帰テストが加わり、PR #404 / #405 のファズキャンペーンがパーサとスパンと fix エンジンに 77 件を積み増した |
...
| 既知バグ | #402 が未修正である。`api.github.com` に届かない環境のオンライン実行が 0.15.2 の 6.2 秒から 0.16.0 で 12.4 秒に倍増する。std の接続プールが死んだ接続を張り直すようになり、`engine.setNetworkDeadline` の 10 秒が `prefetch.zig` の 4 箇所（495 / 681 / 715 / 734 行）でリクエスト間にしか見られないため総時間を縛れていない。autofix 側の #399 は解決済みで、issue の最小入力 3 本と `--iterations 1000 --seed 42` のキャンペーンが `94fd1c9` では 0 failures になる。ファズ由来の先行 6 件（#364 / #366〜#370）も全て解決済みである。クラッシュ（#366）も `--fix` がワークフローを壊す 3 件（#368〜#370）も span 逆転（#367）も空セクションでの lint 不能（#364）も潰れており、形式検証由来の反例と PR #261 で直した 7 件も同様である |
...
| 順 | 対象 | 理由 |
|---|---|---|
| 1 | #402（API 到達不能時の実行時間が倍増） | 唯一残るオープン issue で、利用者が体感する退行である。オフラインではない環境でネットワークが死んでいると 10 秒の期限を超えて 12.4 秒かかる。`prefetch.zig` に転送エラーの短絡を入れる案が小さく、リクエスト単位の期限を残り予算から導く案が期限の意味を回復させる |
| 2 | #217 / #306 | 残る 2 本のドキュメント PR。#217 は形式仕様とモデル検査の反例、#306 は bench 全モードの再実行記録。#306 は別セッションの PR のため本 PR からは触らない |
...
移行後に上がった 2 件のうち #403 は `c802c7f` で解決し、ネットワーク側の #402 だけが残る。
