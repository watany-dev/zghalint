...
オープンな PR / issue を main（`6acaa9a`）の実装状況と突き合わせ、以後の実施順序を示す。
...
| 項目 | 状態 |
|---|---|
| ルール数 | `docs/rules.md` の表は 100 行（SEC023 追加分）で `registry.all_rules` と一致。`src/docs_sync_test.zig`（#242）が ID の欠落・余剰を両方向でテストする。残る不一致は**見出しの「92 rules」だけ**（同期テストは ID 集合のみを見て本文の数字は見ない） |
| `src/**/*.zig` | 57,708 行 |
| ユニットテスト | 1951 件（`zig build test` 緑）。#331 で低品質なテストの削除とテーブル駆動化を行った上で（PR #332）、SC007・ドッグフーディング・形式手法・autofix 拡張・fix の衝突修正・SEC023 の分が積み増している |
...
| E2E テスト | `src/e2e_test.zig` が `tests/fixtures/e2e/*.yml`（67 本）と `tests/fixtures/e2e-action/*.yml`（8 本）の `# zghalint:expect RULE@line` / `forbid` コメントを読んで検証。兄弟ファイル `<name>.fixed` / `<name>.fixed-unsafe` を置いたフィクスチャは `--fix` / `--fix-unsafe` 適用後のバイト列も固定する（PR #328、現在 11 本） |
| bench | `bench/cases/` に 134 ファイル（複数ファイルのケースをまとめて 129 ケース）。A〜D は PR #277、E〜J は PR #287 で揃い、G16 / G17 の堅牢性ケース 3 本が PR #301、G21 の再現ケース 1 本が PR #317、G25 の再現ケース 1 本が PR #357 で加わった。`scripts/bench.py` の採点で zghalint は recall 100%（108/108）・precision 100%・位置一致 96%（104/108）。FN・FP ともに 0 件で、実行エラーは #284 の 1 件のみ。外部ツール未導入の環境では zghalint 単独で採点する。`--perf`（`scripts/bench_perf.py`）が wall time と最大 RSS を 4 シナリオで測り、実ワークフローのコーパスは `scripts/fetch-corpus.py` が取得する（#268、PR #296）。`--fix`（`scripts/bench_fix.py`）は autofix 交差検証で、全ケース × `--fix` / `--fix-unsafe` の 86 適用に対し非冪等 0 / YAML 破損 0 / コメント欠落 0、問題ありは 5 適用（#269、PR #345）。PR #356 が §4.6 の性能表を無効と判定した——zghalint 列が Debug ビルド、actionlint 列が shellcheck 無しという二重の環境差で、ReleaseFast + shellcheck ありで測り直した §4.7 では §4.5 と同じ水準（wall time で 15〜60 倍、RSS で 6〜23 倍の差）に戻る |
...
| オープン PR | #207（本ロードマップ）、#217（形式仕様とモデル検査）、#306（bench 再実行記録）の 3 本。#272 / #277 / #287（bench）と #278（G5〜G8）/ #279（#159）/ #288（G11）/ #289（G12）/ #290（#64）/ #292（G9）/ #291（G14）/ #295（#294）/ #296（#268）/ #301（G16〜G19 の起票）/ #302（#293 = G16 / G17）/ #303（G18 / G19）/ #315（形式手法の Z3 モデル）/ #316（#304 = G20）/ #317（#305 = G21）/ #318（ruff の CI 修正）/ #321（空の値に付いた行末コメントで次のキーを飲み込むパーサバグ）/ #328（#323 = AF1）/ #329（#325 = AF3）/ #330（#324 = AF2）/ #319（action の download 版解決と `scripts/check-version-sync.sh`）/ #332（#331 のテスト整理）/ #338（#307 の F1〜F7）/ #339（#135 = SC007）/ #340（#333 = D1）/ #341（#334 = D2）/ #342（#336 = D4）/ #343（SEC021 の誤検知）/ #345（#269 = bench の autofix 交差検証）/ #350（G22〜G25 の起票）/ #351（#335 = D3）/ #352（#337 = D5）/ #353（#326 / #327 = AF4 / AF5）/ #354（#346〜#348 = G22〜G24）/ #355（#307 の抽出器修復）/ #356（bench の性能再計測と G26〜G28 の起票）/ #357（#349 = G25）/ #361（#286 = G15）はマージ済み。#306 は別セッションの PR のため本 PR からは触らない |
| オープン issue | 6 件。内訳はベンチマーク系 2（#262 本体と運用ループ #270）、bench 由来の parity gap 2（#281 #284 = G10 / G13）、実運用ワークフローから出た新規 gap 2（#359 #360 = G27 / G28。#358 = G26 は解消済み）。形式手法 #307 は PR #355 の抽出器修復で close 済みで、ドッグフーディング（#333〜#337）と autofix 拡張（#322 / #326 / #327）も全て close 済み。#55 #64 #135 #263〜#269 #271 #273〜#276 #280 #282 #283 #285 #293 #294 #297〜#300 #304 #305 #307〜#314 #322〜#327 #331 #333〜#337 #286 #346〜#349 #358 #159 は実装済みのため close 済み |
...
| ベンチマーク #262 | zghalint / actionlint / zizmor を三者比較し、改善課題を継続的に洗い出す umbrella。基盤（#263）は PR #272、ケース A〜D（#264）は PR #277、ケース E〜J（#265〜#267）と FN 候補の検証（#271）は PR #287 で完了した。性能計測（#268）は PR #296 で `--perf` の 4 シナリオとコーパス取得スクリプトが揃い、初回計測で zghalint が actionlint 比 1.5〜3.8 倍・zizmor 比 19〜30 倍速く RSS も最小と出た（副産物として #294 の mmap 過多を発見・修正）。autofix 交差検証（#269）は PR #345 で `scripts/bench.py --fix` として着地し、書き換え後のファイルを zghalint 再実行・actionlint / zizmor・冪等性・YAML 構造の 4 方向から見る。残る sub-issue は運用ループ #270 の 1 本だけで、週次ワークフローでの実行と外部ツールの版固定、parity doc への反映を常設化する課題 |
| bench 由来の parity gap #280〜#286 / #297〜#300 / #304 #305 / #346〜#349 / #358〜#360 | bench と実コーパスで出た FP・FN・実行エラーを `docs/design/external-linter-parity.md` の G9〜G28 として起票したもの。残るのはルール追加 2 本と誤検出・見逃し 3 本——#281（`needs:` の未知 job と循環）/ #284（中身のないワークフローを診断で返す）。#286（G15、API トークンでの publish を trusted publishing へ誘導）は PR #361 で新ルール SEC023 `use-trusted-publishing`（info）として着地し、設計は `docs/adr/0014-sec023-use-trusted-publishing.md` にある。実コーパス由来の FP #305（G21、`actions/checkout` の `path:` が実行時に作るディレクトリへの `uses: ./`）は PR #317 で解消し、ネットワーク不達時に SC003〜SC006 が黙る #304（G20）は PR #316 で解消して `src/rules/net_status.zig` が判定できなかったルールを stderr の注記で伝えるようになった。パーサ層の #293 = #297 / #298（G16 / G17、親キーと同一桁のシーケンスと `-` 単独行）は PR #302、BP001 の reusable workflow 誤検出（G18 = #299）と fix エンジンの二重挿入（G19 = #300）は PR #303 で解消した。ルール改善の #285（G14、PERM001 の過剰警告）は PR #291、パーサ層の #280（G9、関数呼び出し結果へのプロパティ・インデックスアクセス）は PR #292、ファイル全体が検査対象から落ちる系の #282（G11、UTF-8 BOM）は PR #288、#283（G12、ドキュメントマーカー）は PR #289 で解消し、A〜D 由来の G5〜G8（#273〜#276）は PR #278 で解消済み。autofix 交差検証（#269）から G22〜G25（#346〜#349）が PR #350 で起票され、fix 側の欠陥だった 3 本は PR #354 で解消した——SYN009 の `--fix` がタイポを特権トリガ `pull_request_target` / `workflow_run` へ直してしまう #346 は `--fix-unsafe` 送りに、リネーム先が既存キーと衝突して SYN002 を作る #347 は大文字小文字を無視した兄弟キー検査に、SYN001 のリネームと SEC007 の挿入が同じ `permissions:` を二重に作る #348 は同一マッピング内でリネームが生むキーの挿入抑止になった。#349（`*-dependabot.yml` を Dependabot 設定と誤認してワークフロー検査を飛ばす）も PR #357 で解消し、`isDependabotFile` の判定が末尾一致からベース名の完全一致になった（`isActionMetadataFile` と共通の `isDocumentFileNamed` に集約）。さらに実運用ワークフローを流して G26〜G28（#358〜#360）が加わった——#358 は BP003 が第三者アクションの古い major を見逃すもので、埋め込み表が現行 major しか持たず手書きの `deprecated_actions` は `actions/*` 8 件のみという 2 経路の隙間だった（表の最新 major との比較を `info` で足して PR で解消。`docs/adr/0015-bp003-behind-current-major.md`）、#359 は EXPR011 が `${{ }}` で組む動的マトリクスに誤検出を出す、#360 は EXPR007 が値の位置の `||` / `&&` を条件と見なすもの。3 本とも `docs/design/external-linter-parity.md` への追記はまだ入っていない |
| ドッグフーディング #333〜#337（D1〜D5） | 実リポジトリ（honojs/hono など）の `.github/workflows` に zghalint をかけて出た FP とノイズ。D1（#333、EXPR006 の配列 `contains()` 誤検知）は PR #340、D2（#334、PERM002 が workflow-level の `permissions` を無視する）は PR #341、D4（#336、HTTP proxy 環境で SC003 / SC004 / SC005 / SC008 が全て skip される）は PR #342 で解消した。D3（#335、SEC015 と SEC018 が同一 checkout ステップに重複して出る）は PR #351 で解消し、`src/rules/engine.zig` に別ルールが同じ指摘を覆うときの抑制が入った。D5（#337、BP002 が `uses` のみの step にも出て診断の過半を占める）は PR #352 で解消し、BP002 は `run:` の step だけを見るようになった（`uses:` は Actions の UI が action 名でラベルするため。同時に uses 向けの autofix と `stepNameFromRepo` / `stepNameFromRun` が消えた）。D1〜D5 は全て close 済みでこの track に残作業はない。bench が用意したケースで測るのに対し、こちらは実際の利用体験そのものから出てくる |
| autofix 拡張 #322（AF0〜AF5 = #323〜#327） | `--fix` / `--fix-unsafe` に fix を持つのは 100 ルール中 49 ルール（SC007 と SEC023 は fix を持たない）。#322 が全ルールを ◎ / ○ / △ / × で評価し、基盤 1 本あたりの解決ルール数で順を付けた。AF1（#323）は PR #328 で着地し、`src/rules/rename.zig` が did-you-mean 候補を safe な rename fix に変換して SYN001 / SYN009 / SYN010 / SYN016 / SYN019、EXPR010〜EXPR014、PERM003、ACT002 / ACT003 / ACT005、DEP004 / DEP005、RW003 / RW004 の 18 ルールに `--fix` が付いた。AF3（#325）は PR #329 で着地し、`yaml.Sequence` の `ItemDelete` と `fix.builder.deleteSequenceItems` で SYN008 / SYN018（safe）と SYN011 / PERF002（unsafe）が直せる。AF2（#324）は PR #330 で着地し、`graphql` の tag エイリアスが commit oid を運び `src/rules/sha_pin.zig` が SEC001 / SC006 に SHA ピン止め fix を出す（書き換えで意味が変わる場合は抑制）。AF4（#326、挿入系 = BP008 / RW001 / ACT001）と AF5（#327、SEC002 / SEC008 / SEC019 の untrusted 式を `env:` へ束縛する fix）は PR #353 でまとめて着地した——BP008 は `::set-output` 等を環境ファイル書き込みへ置き換える safe な fix、ACT001 / RW001 / SEC002 / SEC008 / SEC019 は値を推論・プレースホルダで補うため unsafe。逆に BP002 は D5（#337、PR #352）で `uses` のみの step を見なくなり、そこに付いていた fix が無くなっている。#322 と #323〜#327 は全て close 済みで、この track に残作業はない |
| 形式手法 #307（F0〜F7 = #308〜#314） | `scripts/formal/` が Z3 の有界モデル検査で SEC ルールの表の抜け漏れを列挙し（`model.py`）、実バイナリで確認する（`confirm.py`）。設計は `docs/design/formal-rule-model.md`。基盤は PR #315 で着地し、F1〜F7（#308〜#314）は PR #338 で全て解消した——SEC005 に `pull_request.merge_commit_sha`、SEC009 に `github.event.workflow_run.pull_requests`、SEC002 / SEC006 / SEC008 に `head.repo.description` / `.homepage` と `*.committer.*` が加わり、SEC021 の ChatOps 判定と SEC002 の env / job outputs 1 ホップ追跡も入った。追跡 issue #307 も PR #355 で close した。`ca923a9` の表分割（`dispatched_inputs_contexts` → `bare_inputs_contexts` + `dispatch_payload_table`）に抽出器が追随できず `LookupError` で落ちていたのを直し、同時に腐敗を検出する経路を CI（`.github/workflows/ci.yml` の "Formal extractor still finds the tables" が z3 なしで `scripts/formal/impl.py` を実行する）と PBT（`tests/pbt/test_formal_extractor.py`）の両方に入れた。F1〜F7 を埋めた時点で `model.py` の証人は P4 の意図的除外 1 件（`workflow_call` + `inputs.*`）だけだった。2026-09-14 に仕様を既知の脆弱性クラスへ広げ（P9〜P12 と P8 の 2 経路: シェルの `git fetch` / `gh pr checkout`、`workflow_run` の artifact poisoning、github-script 以外のコード実行 action 入力、`tj-actions/changed-files` 等の action outputs、ローカル composite action の `inputs`）、実バイナリで確認した 51 件の false negative（意図的除外は P4 / P9 の `workflow_call` × `inputs.*` の 2 件）を G1〜G5（#532〜#536、追跡 #531）として起票した。0.0.1 以降のリリース前に解消する対象。bench が実ワークフローから経験的に穴を探すのに対し、こちらは表の定義から網羅的に探す |
...
| 順 | 対象 | 理由 |
|---|---|---|
| 1 | `docs/rules.md` の見出し修正 | 表は 100 行で registry と同期済みなのに、見出しが「92 rules」のまま。#242 の同期テストは ID 集合しか見ないので本文の数字は守られない |
| 2 | #284 | 中身のないワークフローを `InvalidValue` ではなく診断として返すもので、bench に残る唯一の実行エラー。同じ「ファイルが丸ごと検査から落ちる」系の #349 は PR #357 で解消したため、残るのはこちらだけ |
| 3 | #281 | bench 由来の parity gap で残るルール追加 1 本。`needs:` の未知 job と循環を検出する |
| 4 | #359 / #360 | 実運用ワークフローから出た誤検出 2 本。#359 は動的マトリクス、#360 は値の位置の `||` / `&&`。誤検出は未検出より利用体験を損なうため先に見る。同時に出た #358（BP003 の見逃し）は、BP003 に「表の最新 major より古い major」を `info` で報告する第 3 の判定を足して解消した |
...
実コーパスの FP（#305）は PR #317 で、ドッグフーディングの 5 本（#333〜#337）は PR #340〜#342 / #351 / #352 で全て片付いた。検出そのものの課題として残るのは、ファイルが丸ごと検査から落ちる #284（空ファイル。#349 は PR #357 で解消済み）とルール追加 1 本（#281。#286 は PR #361 の SEC023 で解消）、そして実運用から新たに出た G27 / G28（#359 #360）である。G26（#358、BP003 が第三者アクションの古い major を見逃す）は `popular_actions.latestMajor()` を使う第 3 の判定で解消した。
穴を探す役目は経験的な bench から、表の定義を網羅的に検査する形式手法と、実リポジトリに当てるドッグフーディングへ移った。形式手法は F1〜F7 を PR #338 で出し切って SEC 表の抜けを 7 件埋め、その PR 自身が壊した抽出器も PR #355 で直って CI と PBT の両方から回るようになった。検査手段が CI に載っていない道具は使われないうちに腐る、という教訓がそのまま対策になっている。実運用のワークフローを流す経路は今も新しい穴を出していて、G26〜G28（#358〜#360）がその最新分で、G26 は解消済みである。
検出そのものと並んで、指摘を直せるかどうか（#322 の autofix 拡張。AF1〜AF5 が全て着地し fix を持つルールは 100 中 49 になった）が主戦場になり、fix の正しさを問う G22〜G24 も PR #354 で片付いた。ここに、
