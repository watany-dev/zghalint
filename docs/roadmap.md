...
オープンな PR / issue を main（`61bb873`）の実装状況と突き合わせ、以後の実施順序を示す。
...
| `src/**/*.zig` | 53,998 行 |
| ユニットテスト | 1850 件（`zig build test` 緑） |
...
| E2E テスト | `src/e2e_test.zig` が `tests/fixtures/e2e/*.yml`（52 本）と `tests/fixtures/e2e-action/*.yml`（7 本）の `# zghalint:expect RULE@line` / `forbid` コメントを読んで検証。兄弟ファイル `<name>.fixed` を置いたフィクスチャは `--fix` 適用後のバイト列も固定する（PR #328） |
...
| オープン PR | #207（本ロードマップ）、#217（形式仕様とモデル検査）、#306（bench 再実行記録）、#319（action の download 版解決）、#320（v0.2 マルチ CI ロードマップ = ADR-0014 + 設計 doc）、#330（#324 = AF2 の基盤、tag alias に commit oid を載せる）の 6 本。#272 / #277 / #287（bench）と #278（G5〜G8）/ #279（#159）/ #288（G11）/ #289（G12）/ #290（#64）/ #292（G9）/ #291（G14）/ #295（#294）/ #296（#268）/ #301（G16〜G19 の起票）/ #302（#293 = G16 / G17）/ #303（G18 / G19）/ #315（形式手法の Z3 モデル）/ #316（#304 = G20）/ #317（#305 = G21）/ #318（ruff の CI 修正）/ #321（空の値に付いた行末コメントで次のキーを飲み込むパーサバグ）/ #328（#323 = AF1）/ #329（#325 = AF3）はマージ済み。#306 / #319 / #320 / #330 は別セッションの PR のため本 PR からは触らない |
| オープン issue | 20 件。内訳はその他 1（#135）、テスト整理 1（#331、テーブル駆動化でコンパイル時間を削る）、autofix 拡張 4（#322 の追跡 issue と #324 / #326 / #327 = AF2 / AF4 / AF5）、形式手法 8（#307 の追跡 issue と #308〜#314 = F1〜F7）、ベンチマーク系 3（#262 本体と #269 / #270）、bench 由来の parity gap 3（#281 #284 #286 = G10 / G13 / G15）。#55 #64 #263 #264 #265〜#267 #268 #271 #273〜#276 #280 #282 #283 #285 #293 #294 #297〜#300 #304 #305 #323 #325 #159 は実装済みのため close 済み |
...
| autofix 拡張 #322（AF0〜AF5 = #323〜#327） | `--fix` / `--fix-unsafe` に fix を持つのは 98 ルール中 42 ルール。#322 が全ルールを ◎ / ○ / △ / × で評価し、基盤 1 本あたりの解決ルール数で順を付けた。AF1（#323）は PR #328 で着地し、`src/rules/rename.zig` が did-you-mean 候補を safe な rename fix に変換して SYN001 / SYN009 / SYN010 / SYN016 / SYN019、EXPR010〜EXPR014、PERM003、ACT002 / ACT003 / ACT005、DEP004 / DEP005、RW003 / RW004 の 18 ルールに `--fix` が付いた。AF3（#325）は PR #329 で着地し、`yaml.Sequence` の `ItemDelete` と `fix.builder.deleteSequenceItems` で SYN008 / SYN018（safe）と SYN011 / PERF002（unsafe）が直せる。残るのは AF2（#324、`tag_oids` を使った SHA ピン止め = SEC001 / SC006。基盤の PR #330 がオープン）/ AF4（#326、挿入系 = BP008 / RW001 / ACT001）/ AF5（#327、SEC002 / SEC008 / SEC019 の env 束縛。設計 doc 先行） |
...
| 5 | #322 の AF2 | SHA ピン止め fix。基盤の PR #330 が tag alias に commit oid を載せるところまで来ており、SEC001 / SC006 という最も指摘数の多い 2 ルールが直せるようになる |
...
検出そのものと並んで、指摘を直せるかどうか（#322 の autofix 拡張。AF1 / AF3 の着地で fix を持つルールは 98 中 42 に増えた）と、
