...
| オープン issue | 7 件。内訳はベンチマーク系 2（#262 本体と運用ループ #270）、bench 由来の parity gap 2（#281 #284 = G10 / G13）、実運用ワークフローから出た新規 gap 3（#358〜#360 = G26 / G27 / G28）。形式手法 #307 は PR #355 の抽出器修復で close 済みで、ドッグフーディング（#333〜#337）と autofix 拡張（#322 / #326 / #327）も全て close 済み。#55 #64 #135 #263〜#269 #271 #273〜#276 #280 #282 #283 #285 #293 #294 #297〜#300 #304 #305 #307〜#314 #322〜#327 #331 #333〜#337 #286 #346〜#349 #159 は実装済みのため close 済み |
...
| 4 | #359 / #360 | 実運用ワークフローから出た誤検出 2 本。#359 は動的マトリクス、#360 は値の位置の `||` / `&&`。誤検出は未検出より利用体験を損なうため、同時に出た #358（BP003 の見逃し）より先に見る |
...
実コーパスの FP（#305）は PR #317 で、ドッグフーディングの 5 本（#333〜#337）は PR #340〜#342 / #351 / #352 で全て片付いた。検出そのものの課題として残るのは、ファイルが丸ごと検査から落ちる #284（空ファイル。#349 は PR #357 で解消済み）とルール追加 1 本（#281。#286 は PR #361 の SEC023 で解消）、そして実運用から新たに出た G26〜G28（#358〜#360）である。
穴を探す役目は経験的な bench から、表の定義を網羅的に検査する形式手法と、実リポジトリに当てるドッグフーディングへ移った。形式手法は F1〜F7 を PR #338 で出し切って SEC 表の抜けを 7 件埋め、その PR 自身が壊した抽出器も PR #355 で直って CI と PBT の両方から回るようになった。検査手段が CI に載っていない道具は使われないうちに腐る、という教訓がそのまま対策になっている。実運用のワークフローを流す経路は今も新しい穴を出していて、G26〜G28（#358〜#360）がその最新分である。
