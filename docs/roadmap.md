...
オープンな PR / issue を main（`7c09887`）の実装状況と突き合わせ、以後の実施順序を示す。
...
| bench 由来の parity gap #280〜#286 / #297〜#300 / #304 #305 / #346〜#349 | bench と実コーパスで出た FP・FN・実行エラーを `docs/design/external-linter-parity.md` の G9〜G25 として起票したもの。残るのはルール追加 3 本——#281（`needs:` の未知 job と循環）/ #284（中身のないワークフローを診断で返す）/ #286（API トークンでの publish を trusted publishing へ誘導）。実コーパス由来の FP #305（G21、`actions/checkout` の `path:` が実行時に作るディレクトリへの `uses: ./`）は PR #317 で解消し、ネットワーク不達時に SC003〜SC006 が黙る #304（G20）は PR #316 で解消して `src/rules/net_status.zig` が判定できなかったルールを stderr の注記で伝えるようになった。パーサ層の #293 = #297 / #298（G16 / G17、親キーと同一桁のシーケンスと `-` 単独行）は PR #302、BP001 の reusable workflow 誤検出（G18 = #299）と fix エンジンの二重挿入（G19 = #300）は PR #303 で解消した。ルール改善の #285（G14、PERM001 の過剰警告）は PR #291、パーサ層の #280（G9、関数呼び出し結果へのプロパティ・インデックスアクセス）は PR #292、ファイル全体が検査対象から落ちる系の #282（G11、UTF-8 BOM）は PR #288、#283（G12、ドキュメントマーカー）は PR #289 で解消し、A〜D 由来の G5〜G8（#273〜#276）は PR #278 で解消済み。autofix 交差検証（#269）から G22〜G25（#346〜#349）が PR #350 で起票され、fix 側の欠陥だった 3 本は PR #354 で解消した——SYN009 の `--fix` がタイポを特権トリガ `pull_request_target` / `workflow_run` へ直してしまう #346 は `--fix-unsafe` 送りに、リネーム先が既存キーと衝突して SYN002 を作る #347 は大文字小文字を無視した兄弟キー検査に、SYN001 のリネームと SEC007 の挿入が同じ `permissions:` を二重に作る #348 は同一マッピング内でリネームが生むキーの挿入抑止になった。残るのは #349（`*-dependabot.yml` を Dependabot 設定と誤認してワークフロー検査を飛ばす）1 本 |
...
| 形式手法 #307（F0〜F7 = #308〜#314） | `scripts/formal/` が Z3 の有界モデル検査で SEC ルールの表の抜け漏れを列挙し（`model.py`）、実バイナリで確認する（`confirm.py`）。設計は `docs/design/formal-rule-model.md`。基盤は PR #315 で着地し、F1〜F7（#308〜#314）は PR #338 で全て解消した——SEC005 に `pull_request.merge_commit_sha`、SEC009 に `github.event.workflow_run.pull_requests`、SEC002 / SEC006 / SEC008 に `head.repo.description` / `.homepage` と `*.committer.*` が加わり、SEC021 の ChatOps 判定と SEC002 の env / job outputs 1 ホップ追跡も入った。ただし追跡 issue #307 は閉じられない。ハーネス側が main で動かなくなっているためで、`ca923a9` が `security.zig` の `dispatched_inputs_contexts` を `bare_inputs_contexts` と `dispatch_payload_table` に分割した結果、旧名を正規表現で探す `scripts/formal/impl.py:95` が `LookupError: the extractor is out of date` で落ちる（`model.py` も `confirm.py` も同じ `impl.load()` を通るため両方動かない）。抽出器が黙って空表を返さず落ちる設計は正しく働いているが、`scripts/formal/` を回すジョブが CI に無いため、F1〜F7 を埋めた当の PR が抽出器を壊したまま通った。残作業は抽出の追随と、証人が再び出ないことの確認、そして最低限 `impl.load()` の疎通を CI か PBT から回すこと。bench が実ワークフローから経験的に穴を探すのに対し、こちらは表の定義から網羅的に探す |
...
| 順 | 対象 | 理由 |
|---|---|---|
| 1 | #307 の抽出器修復 | `scripts/formal/impl.py` が `security.zig` の表の改名に追随できず落ちている。F1〜F7 を埋めた結果が本当に反映されたかを確認する手段が今は無い。CI に載っていないことが腐敗を許した原因なので、修復と同時に疎通だけでも回す |
| 2 | `docs/rules.md` の見出し修正 | 表は 99 行で registry と同期済みなのに、見出しが「91 rules」のまま。#242 の同期テストは ID 集合しか見ないので本文の数字は守られない |
| 3 | #284 / #349 | ファイルが検査対象から落ちる系 2 本。#284 は中身のないワークフローを `InvalidValue` ではなく診断として返すもので bench に残る唯一の実行エラー、#349 は `*-dependabot.yml` を Dependabot 設定と誤認してワークフロー検査を飛ばすもの |
...
実コーパスの FP（#305）は PR #317 で、ドッグフーディングの 5 本（#333〜#337）は PR #340〜#342 / #351 / #352 で全て片付いた。検出そのものの課題として残るのは、ファイルが丸ごと検査から落ちる 2 本（#284 の空ファイルと #349 の `*-dependabot.yml`）とルール追加 2 本（#281 / #286）である。
穴を探す役目は経験的な bench から、表の定義を網羅的に検査する形式手法と、実リポジトリに当てるドッグフーディングへ移った。形式手法は F1〜F7 を PR #338 で出し切って SEC 表の抜けを 7 件埋めたが、その PR 自身が抽出器を壊しており（#307）、検査手段が CI に載っていない道具は使われないうちに腐るという教訓を残した。
