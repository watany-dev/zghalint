...
オープンな PR / issue を main（`a156b3b`）の実装状況と突き合わせ、以後の実施順序を示す。
...
| `src/**/*.zig` | 81 ファイル 66,283 行 |
| ユニットテスト | 2223 件（`zig build test` 緑）。#331 で低品質なテストの削除とテーブル駆動化を行った上で（PR #332）、SC007・ドッグフーディング・形式手法・autofix 拡張・fix の衝突修正・SEC023・SYN020・EXPR007 / EXPR011 の誤検出修正・性能最適化・空セクションのパース継続・SC003 のピンコメント版判定・BP007 のプロセス置換・SEC023 の crates.io・EXPR011 のオブジェクト軸・SYN021 / SYN022・BP003 の最新 major 比較・SEC016 の暗黙キャッシュ判定の分が積み増している。Zig 0.16 移行では stdout / stderr のリダイレクト保持と atomic replace の権限復元に回帰テストが加わり、PR #404 / #405 のファズキャンペーンがパーサとスパンと fix エンジンに 77 件を積み増した。GA トラック第 1 段階の PR #442〜#445 が SYN023・EXPR005 の偶数引数・RUNNER001 の macos-13 retired 化の分を、第 2 段階の PR #446 / #447 が `background` / `wait` / `parallel` の AST と SYN024・EXPR019 の分を足している。過剰設計監査の PR #498 は重複の集約で 602 行を落としながらテストを 4 件増やしており、`src/test_support.zig` へ寄せたルールテストの補助関数と `src/rules/ref_cache.zig` / `src/rules/expr_check.zig` の共通ヘルパがその受け皿である。第 3 段階の PR #448 が `cache-mode` の能力モデルと SEC024 の分を積み増した。過剰設計監査の PR #500 / #501 / #503 / #505 / #509 / #510 はネットワーク層と `glob.zig` と公開 API の整理で、振る舞いを変えないため件数は動かない |
...
| E2E テスト | `src/e2e_test.zig` が `tests/fixtures/e2e/*.yml`（95 本）と `tests/fixtures/e2e-action/*.yml`（8 本）の `# zghalint:expect RULE@line` / `forbid` コメントを読んで検証。兄弟ファイル `<name>.fixed` / `<name>.fixed-unsafe` を置いたフィクスチャは `--fix` / `--fix-unsafe` 適用後のバイト列も固定する（PR #328、現在 17 本） |
...
| 過剰設計監査 #449（PT0〜PT45 = #450〜#494） | `ponytail-audit` によるリポジトリ全体の監査から起票した umbrella。対象は死んだコード・std が既に持つ自前実装・実装が 1 つしかない抽象・誰も設定しない config・同じ論理をより短く書ける箇所に限り、正しさ・セキュリティ・性能は通常のレビューへ回す。45 本の子 issue のうち 25 本が PR #495〜#498 / #500 / #501 / #503 / #505 / #509 / #510 で着地し、17 本は精査の結果 not planned、3 本が残る。着地分は重複の集約（PT6 = `RefCache`、PT7 = `test_support.zig`、PT8 = `expr_check.Segment.name()`、PT12 = did-you-mean 候補文、PT23 = `findEntry`、PT24 = `hasEmptySection`、PT30 = `checkKind`、PT33 = comptime フラグ、PT38 = `appendUnique`）、std への置換（PT15 = `stringToEnum` / `@tagName`、PT16 = `eqlIgnoreCase`、PT21 = disk_cache のコーデック、PT42 = `EnumSet`、PT43 = `std.mem.find`）、ツーリングと CI（PT4 / PT9 / PT14 / PT20 / PT45）、ネットワーク層と公開 API の整理（PT22 = `PermissionsMeta` の comptime 導出、PT28 = 無制限 `fetch` の削除、PT29 = `glob.zig` の private arena、PT39 = `prefetch` の sha_pin 再エクスポート、PT40 = `standard_headers` の定数配列化、PT44 = 外部参照のない `pub` の private 化）である。残る 3 本は PT25 / PT27 / PT34 で、3 本とも PR #506 / #504 / #502 が審査中である。各変更で `zig build && zig fmt --check src/ build.zig && zig build test --summary all` が通り、診断の増減が出ないことが完了条件で、削減が大きくても根拠のあるもの——`scripts/formal/` の Z3 モデル一式（PT1）、`src/fuzz_driver.zig` と `fuzz-driver` ステップ（PT2）、`bench_perf.py` の自前計測バックエンド（PT3）、`fix/builder.zig` の memcpy 系挿入（PT5）、`timezones.zig` の IANA 表・`cron.zig` の暦計算・`http_client` の `BoundedBody` と `SocketReaper`・YAML のアンカーとマージキー・bench スクリプトの分割——は残す判断が下りている |
...
| 順 | 対象 | 理由 |
|---|---|---|
| 1 | #421（G37。PR なし） | 唯一の検出漏れを含むドッグフーディング由来のバグ。`src/yaml/tokenizer.zig` の `scanPlainScalar` と `skipExpressionInterpolation` が plain scalar の行継続を読まないため、正当な式を EXPR001 にし、継続行側の汚染源を SEC002 が見落とす。FP と FN の両方が同じ欠落から出るので、直す価値が最も高い |
| 2 | #502〜#515 のレビュー | 未解決 issue 18 件のうち 10 件に PR が立っている。過剰設計監査の 3 本（#502 / #504 / #506）、検出バグの 3 本（#507 / #508 / #515）、autofix P1 の 4 本（#511〜#514）で、いずれも main へ取り込めば対応 issue が閉じる。実装よりレビューの通過が律速である |
| 3 | #435〜#438（GA8〜GA11） | GA トラック第 3 段階の残り。GA7 が `cache-mode` を 2 能力へ解決する resolver を入れたので、setup-node の自動キャッシュ・checkout の credential・node20 の deprecated 化・concurrency の矛盾も同じ capability の形に載る |
| 4 | #414〜#417（AF11〜AF14） | autofix 第 2 波の残り。`secrets: inherit` の展開と SC003 の `patched_version` bump は既存基盤で届くが、SEC002 の github-script `script:` と SC001 のコンテナ digest ピンは JS 書き換えとレジストリ I/O のため別 ADR を要する |
...
過剰設計監査（#449 = PT0）の残る 3 件は振る舞いを変えない整理で、検出バグや GA トラックとは独立に進められる。3 件すべてに PR #506 / #504 / #502 が立っており、未着手のものはない。
