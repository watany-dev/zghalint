# 実施ロードマップ（2026-09-13 時点）

オープンな PR / issue を main（`aecc0a2`）の実装状況と突き合わせ、以後の実施順序を示す。
...
| 項目 | 状態 |
|---|---|
| ルール数 | `docs/rules.md` の表は 108 行（SYN023 の `cache-mode` 値検証、SYN024 の `wait` 参照先 step id 検証、SYN026 の GitHub Actions が受理しない YAML merge key `<<`、EXPR019 の未同期 background 出力参照、SEC024 の低信頼トリガー上の `cache-mode: write` を含む）で `registry.all_rules` と一致。`src/docs_sync_test.zig`（#242）が ID の欠落・余剰を両方向でテストする。同期テストは ID 集合しか見ないため見出しの数字は手で守る必要があり、現在の見出しは 108 で表と揃っている
| `src/**/*.zig` | 84 ファイル 67,824 行 |
| ユニットテスト | 2289 件（`zig build test` 緑）。#331 で低品質なテストの削除とテーブル駆動化を行った上で（PR #332）、SC007・ドッグフーディング・形式手法・autofix 拡張・fix の衝突修正・SEC023・SYN020・EXPR007 / EXPR011 の誤検出修正・性能最適化・空セクションのパース継続・SC003 のピンコメント版判定・BP007 のプロセス置換・SEC023 の crates.io・EXPR011 のオブジェクト軸・SYN021 / SYN022・BP003 の最新 major 比較・SEC016 の暗黙キャッシュ判定の分が積み増している。Zig 0.16 移行では stdout / stderr のリダイレクト保持と atomic replace の権限復元に回帰テストが加わり、PR #404 / #405 のファズキャンペーンがパーサとスパンと fix エンジンに 77 件を積み増した。GA トラック第 1 段階の PR #442〜#445 が SYN023・EXPR005 の偶数引数・RUNNER001 の macos-13 retired 化の分を、第 2 段階の PR #446 / #447 が `background` / `wait` / `parallel` の AST と SYN024・EXPR019 の分を足している。過剰設計監査の PR #498 は重複の集約で 602 行を落としながらテストを 4 件増やしており、`src/test_support.zig` へ寄せたルールテストの補助関数と `src/rules/ref_cache.zig` / `src/rules/expr_check.zig` の共通ヘルパがその受け皿である。第 3 段階の PR #448 が `cache-mode` の能力モデルと SEC024 の分を積み増した。過剰設計監査の PR #500〜#506 / #509 / #510 はネットワーク層と `glob.zig` と公開 API の整理で、振る舞いを変えないため件数は動かない。autofix 第 2 波の P1 4 本（PR #511〜#514）と検出バグ 4 本（PR #507 / #508 / #515 / #516）の分がここに積み増している。GA トラックの PR #517（GA8 = setup-node の自動キャッシュ）/ #523（GA9 = checkout の credential と unsafe-pr-checkout の capability 化）/ #518（GA10 = node20 の deprecated 化）/ #520（GA12 = SYN026）が直近の増分である |
...
| E2E テスト | `src/e2e_test.zig` が `tests/fixtures/e2e/*.yml`（118 本）と `tests/fixtures/e2e-action/*.yml`（10 本）の `# zghalint:expect RULE@line` / `forbid` コメントを読んで検証。兄弟ファイル `<name>.fixed` / `<name>.fixed-unsafe` を置いたフィクスチャは `--fix` / `--fix-unsafe` 適用後のバイト列も固定する（PR #328、現在 `tests/fixtures/e2e/` に 29 本、`tests/fixtures/e2e-action/` に 5 本） |
| bench | `bench/cases/` に 147 ファイル（複数ファイルのケースをまとめて 142 ケース）。A〜D は PR #277、E〜J は PR #287 で揃い、G16 / G17 の堅牢性ケース 3 本が PR #301、G21 の再現ケース 1 本が PR #317、G25 の再現ケース 1 本が PR #357、G36 の再現ケース 1 本が PR #515、G37 の再現ケース 2 本が PR #516 で加わった。`scripts/bench.py` の採点で zghalint は recall 100%（117/117）・precision 100%・位置一致 97%（113/117）、実行エラー 0 件。FP も FN も 0 件で、`bench/baseline.json` に記録された既知 gap は残っていない。skip はネットワークを要する 2 ケースと、SEC002 が matrix.* を対象外にする意図的な非対応 1 ケースだけである。外部ツール未導入の環境では zghalint 単独で採点する。採点結果は `bench/baseline.json` に固定し、`scripts/bench_gate.py` が回帰を非ゼロ終了で落とす（#270、PR #373）。`.github/workflows/bench.yml` が週次で 3 モードを回して parity doc へ流す。`--perf`（`scripts/bench_perf.py`）が wall time と最大 RSS を 4 シナリオで測り、実ワークフローのコーパスは `scripts/fetch-corpus.py` が取得する（#268、PR #296）。`--fix`（`scripts/bench_fix.py`）は autofix 交差検証で、非冪等 0 / YAML 破損 0 / コメント欠落 0。意図した増加は各ケースの `# bench:fix-allow <flag> <tool>=<ID> <理由>` で宣言し、宣言のないものだけが問題として残る（#269、PR #345、宣言形式は PR #373）。PR #356 が §4.6 の性能表を無効と判定した——zghalint 列が Debug ビルド、actionlint 列が shellcheck 無しという二重の環境差で、ReleaseFast + shellcheck ありで測り直した §4.7 では §4.5 と同じ水準（wall time で 15〜60 倍、RSS で 6〜23 倍の差）に戻る |
...
| オープン PR | 7 本。内訳は GA トラック第 3 段階の実装 1 本（#519 = GA11 / #438）、第 4 段階の実装 2 本（#521 = GA13 / #440、#522 = GA14 / #441）、ドッグフーディングの起票 1 本（#426 = G38 / G39）、#217（形式仕様とモデル検査）、#306（bench 再実行記録）、#395（本ロードマップの同期）である。GA トラック第 3 段階の #517（GA8）/ #523（GA9）/ #518（GA10）と第 4 段階の #520（GA12）は `aecc0a2` までにマージ済みで、対応する issue #435 / #436 / #437 / #439 も close 済みである。ドッグフーディング起票の #420（G36）/ #422（G37）はマージ済みである。過剰設計監査の 10 本（#500〜#506 / #509 / #510）、検出バグの 4 本（#507 = G39 / #425、#508 = G38 / #424、#515 = G36 / #419、#516 = G37 / #421）、autofix 第 2 波 P1 の 4 本（#511 = AF8 / #411、#512 = AF9 / #412、#513 = AF10 / #413、#514 = AF7 / #410）、#499（dependabot による ruff 0.15.8 → 0.16.6）はいずれも `a12d89a` までにマージ済みで、対応 issue も close 済みである。#418（`util` の `IgnoreCaseMap`）はマージされずに close された。GA トラック第 1 段階の P0 実装 4 本——#442（GA1 = `cache-mode` / `permissions.vulnerability-alerts` / `job.workflow_*`）/ #443（GA2 = `case()` の奇数引数）/ #444（GA3 = macos-13 の retired 化）/ #445（GA4 = 比較基準バージョンの更新）——はマージ済みで、対応する issue #428〜#431 も close 済みである。第 2 段階の #446（GA5 = `background` / `wait` / `parallel` の AST と再帰 lint 走査、SYN024）/ #447（GA6 = 未同期 background 出力参照の EXPR019）もマージ済みで、issue #432 / #433 は close 済みである。#392（#375 = D7）/ #393（#382 = G30）/ #394（#281 = G10、SYN021 / SYN022 追加）/ #396（#358 = G26 の BP003 最新 major 比較）/ #397（SEC016 の暗黙キャッシュと PERF001 の逆向き助言の抑制）/ #400（Zig 0.16 移行と `std.Io` 採用）/ #401（0.0.1-rc.2 へのバージョン更新）/ #404 / #405（ファズキャンペーンによるパーサ・スパン・fix エンジンの修正と #403 の memset 除去）/ #407（引用状態の再走査を最終行だけに絞る YAML パーサの性能修正）/ #408（#402 のネットワーク fail fast とリクエスト単位の予算）/ #423（リリース版を 0.0.1 へ）もマージ済みである。本ロードマップの #207 は `3951cc7` でマージ済みで、以後の同期はこのブランチが引き継ぐ。過剰設計監査からは #495（PT4 = `update-design` スキルの削除）/ #497（PT9 / PT14 / PT20 / PT45 = bench フラグ・CI の build ジョブ・Homebrew formula テストの pytest 化・pyyaml の二重ピン）/ #498（PT6 / PT7 / PT8 / PT12 / PT23 / PT24 / PT30 / PT33 / PT38 ほかの重複集約）/ #496（PT15 / PT16 / PT21 / PT42 / PT43 = std への置換）がマージ済みである。GA トラック第 3 段階の #448（GA7 = `cache-mode` の能力モデルと SEC016 / PERF001 共通化、SEC024 追加）もマージ済みで、issue #434 は close 済みである。#362（#284 = G13）/ #363（#359 / #360 = G27 / G28）/ #365（性能最適化）/ #371（ファズドライバと #364〜#370 の起票）/ #373（#270 = bench 運用ループ）/ #374（install.sh と Homebrew tap）/ #376（#366 = taint テーブル溢れ）/ #377（perf bench に ghalint / octoscan / poutine / action-validator を追加）/ #379（SEC013 の GHCR login と BP007 の PowerShell 代入の FP、G29 の起票）/ #385（G30〜G32 の起票）/ #387（G33 の起票）/ #380（#367〜#370 の span 逆転と fix 非収束）/ #389（G31〜G33 = #383 #384 #386）/ #390（#364 の空セクション）/ #391（D6 = #372 の SC003 / SC005 / Release / skip 注記）も既にマージされている。#306 は別セッションの PR のため本 PR からは触らない |
| オープン issue | 9 件。内訳は v0.2.0 の GitHub Actions 最新仕様追従（GA0 = #427、GA11 = #438、GA13 = #440、GA14 = #441）が 4 件、autofix 第 2 波（AF6 = #409 と AF11〜AF14 = #414〜#417）が 5 件である。ドッグフーディング由来の検出バグは 0 件で、過剰設計監査（#449）の残りも 0 件である。#449 は `ponytail-audit` がリポジトリ全体を過剰設計の観点で洗った追跡 issue で、45 本の子 issue は完了 25 本・精査の結果 not planned 17 本ですべて閉じ、umbrella も close 済みである。振る舞いを変えないことが前提のため、削減が大きくても正当性の根拠がある PT1（`scripts/formal/` の Z3 モデル一式）・PT2（`src/fuzz_driver.zig` と `fuzz-driver` ステップ）・PT3（`bench_perf.py` の自前計測バックエンド）・PT5（`fix/builder.zig` の memcpy 系挿入）は not planned で閉じ、対象のコードは残っている。#427 が GA トラックの追跡 issue で、第 1 段階の P0 4 件（#428〜#431）は PR #442〜#445 で、第 2 段階の 2 件（#432 / #433）は PR #446 / #447 で着地して close 済みである——`syntax.zig` が `cache-mode` を SYN023 として値まで検証し、`permissions.vulnerability-alerts` と `job.workflow_ref` / `workflow_sha` が語彙に入り、`expr_catalog.zig` の `case` は奇数引数だけを受理し、`runner.zig` は `macos-13` を `.retired` にして `macos-11` / `macos-12` の replacement を `macos-15` へ付け替え、比較基準は actionlint 1.7.12 / zizmor 1.30.1 に固定された。`types.zig` / `parser.zig` / `schema.zig` が `background` / `wait` / `parallel` を AST に持ち、`parallel` の内側まで lint が再帰し、`src/rules/background_sync.zig` が `wait` 前の outputs 参照を EXPR019 にする。第 3 段階の GA7（#434）も PR #448 で着地して close 済みで、`types.zig` が `cache-mode` を `can_restore` / `can_save` の 2 能力へ解決し、SEC016 と PERF001 が同じ resolver を共有し、低信頼トリガー上の `cache-mode: write` / `write-only` を SEC024 が警告する（autofix なし）。第 3 段階の GA8（#435）/ GA9（#436）/ GA10（#437）は PR #517 / #523 / #518 で、第 4 段階の GA12（#439）は PR #520 で着地して close 済みである——`setup-node` の `cache` 省略時の自動キャッシュを PERF001 / SEC016 の判定に入れ、`checkout` の credential 保存場所と unsafe な PR チェックアウトを capability として扱い、`node20` を deprecated runtime として診断し、GitHub Actions が受理しない YAML merge key `<<` を SYN026 が検出する。残るのは第 3 段階の #438 と、第 4 段階の #440 / #441 である。#409 が autofix の追跡 issue で、P1 の 4 件（#410 = EXPR002 / EXPR003 / EXPR004 の did-you-mean rename、#411 = ACT001 composite の `shell:` 挿入、#412 = SYN012 排他フィルタの後勝ちキー削除、#413 = SEC014 bot 判定の `sender.type` 比較）は PR #514 / #511 / #512 / #513 で着地して close 済みである。残る 4 件は #414（SEC010 の `secrets: inherit` 展開）、#415（SC003 の `patched_version` へ bump）、#416（SEC002 の github-script `script:`）、#417（SC001 のコンテナイメージ digest ピン）で、後 2 件は JS 書き換えとレジストリ I/O のため別 ADR を要する。ドッグフーディング由来の検出バグは 4 件とも閉じている——#421（G37 = 行をまたぐ plain scalar の `${{ }}` を未閉じにする EXPR001 の FP と、継続行側の汚染源に対する SEC002 の FN）は PR #516、#419（G36 = 真偽値関数の結果だけを `run:` へ展開する形での SEC002 の FP）は PR #515、#424（G38 = コンテキストパス上の数値インデックスを EXPR001 にする FP）は PR #508、#425（G39 = ローカル `uses:` の `@scope` を ref と誤認する DEP003 の FP）は PR #507 で閉じた。issue を持たない追跡対象は SEC016 の暗黙キャッシュ FN だけである |
...
| 既知バグ | 未修正は 0 件である。ドッグフーディングが実運用の CI から掘り出した 4 件はすべて閉じた——#421（`scanPlainScalar` と `skipExpressionInterpolation` が plain scalar の行継続を読まないことによる EXPR001 の FP と SEC002 の FN）は PR #516、#419（真偽値関数の結果だけが `run:` に届く形での SEC002 の FP）は PR #515、#424（`parseContextAccess` がブラケット内を文字列リテラルに限る FP）は PR #508、#425（`actionProblem` がローカル参照中の `@` を位置に関係なく ref とみなす FP）は PR #507 である。仕様追従の遅れから来ていた検出漏れ 2 件（#429 = EXPR005 が `case()` の偶数引数を通す、#430 = RUNNER001 が廃止済みの `macos-13` を current 扱いし `--fix-unsafe` の誘導先にもする）は PR #443 / #444 で解決した。#402 は PR #408 が閉じた——`engine` がネットワーク期限からリクエスト単位の予算を導き、`http_client` が最初の transport 失敗で fail fast して in-flight のソケットを落とし、`prefetch` は到達不能を検出したら REST フォールバックを飛ばす。パケットを落とすプロキシ越しの `--no-cache` 実行が 134.5 秒から 5.1 秒になる。#399 も解決済みで、issue の最小入力 3 本と `--iterations 1000 --seed 42` のキャンペーンが 0 failures になる。ファズ由来の先行 6 件（#364 / #366〜#370）も同様である。クラッシュ（#366）も `--fix` がワークフローを壊す 3 件（#368〜#370）も span 逆転（#367）も空セクションでの lint 不能（#364）も潰れており、形式検証由来の反例と PR #261 で直した 7 件も同様である |
...
| GitHub Actions 仕様追従 #427（GA0〜GA14 = #428〜#441） | 2026-09-11 の仕様追従レポート（調査範囲 2024-09-11〜2026-09-11、比較対象 actionlint 1.7.12 / zizmor 1.30.1 / ghalint 1.5.6）から起票された v0.2.0 の umbrella。狙いはルール数を増やすことではなく、正しい新構文を誤検出しないこと・受理した構文を最後まで検査すること・古い知識に基づく誤修正をなくすことの 3 点である。第 1 段階の P0（#428〜#431）は PR #442〜#445 で着地して 4 件とも close 済みで、`cache-mode` / `permissions.vulnerability-alerts` / `job.workflow_*` の受理と SYN023 による値検証、EXPR005 の `case()` 奇数引数、RUNNER001 の macos-13 retired 化と replacement 不変条件、比較基準バージョンの更新が入った。第 2 段階（#432 / #433）は PR #446 / #447 で着地して 2 件とも close 済みで、`background` / `wait` / `parallel` が `types.zig` / `parser.zig` / `schema.zig` の AST に入り、`parallel` の内側まで lint が再帰し、`wait` の参照先 step id を SYN024 が、`wait` 前の outputs 参照を `src/rules/background_sync.zig` の EXPR019 が見る。第 3 段階（#434〜#438）は cache / checkout を capability として持ち、SEC016 と PERF001 の判定をその上へ寄せた上で、setup-node の自動キャッシュ・node20 の deprecated 診断・`concurrency.queue` の矛盾検出を足す。うち GA7（#434）/ GA8（#435）/ GA9（#436）/ GA10（#437）は PR #448 / #517 / #523 / #518 で着地して close 済みで、残るのは `concurrency.queue` の矛盾を見る GA11（#438、PR #519）である。第 4 段階（#439〜#441）は未対応 YAML merge key・`prefer-self-repository`・仕様追従用フィクスチャカタログで、追従漏れ自体を検出する側の整備である。うち GA12（#439）は PR #520 で SYN026 として着地して close 済みで、残るのは #440 / #441 である。意味が変わる書き換えは `--fix` にしない方針が umbrella に明記されている |
| 過剰設計監査 #449（PT0〜PT45 = #450〜#494） | `ponytail-audit` によるリポジトリ全体の監査から起票した umbrella で、45 本の子 issue がすべて閉じたため umbrella も close 済み。対象は死んだコード・std が既に持つ自前実装・実装が 1 つしかない抽象・誰も設定しない config・同じ論理をより短く書ける箇所に限り、正しさ・セキュリティ・性能は通常のレビューへ回した。25 本が PR #495〜#498 / #500〜#506 / #509 / #510 で着地し、17 本は精査の結果 not planned である。着地分は重複の集約（PT6 = `RefCache`、PT7 = `test_support.zig`、PT8 = `expr_check.Segment.name()`、PT12 = did-you-mean 候補文、PT23 = `findEntry`、PT24 = `hasEmptySection`、PT30 = `checkKind`、PT33 = comptime フラグ、PT38 = `appendUnique`）、std への置換（PT15 = `stringToEnum` / `@tagName`、PT16 = `eqlIgnoreCase`、PT21 = disk_cache のコーデック、PT42 = `EnumSet`、PT43 = `std.mem.find`）、ツーリングと CI（PT4 / PT9 / PT14 / PT20 / PT45）、ネットワーク層と公開 API の整理（PT22 = `PermissionsMeta` の comptime 導出、PT25 = 1 行フォワーダ、PT27 = `rest_fallback.TagResolution` への一本化、PT28 = 無制限 `fetch` の削除、PT29 = `glob.zig` の private arena、PT34 = `disk_cache` の `cache_format`、PT39 = `prefetch` の sha_pin 再エクスポート、PT40 = `standard_headers` の定数配列化、PT44 = 外部参照のない `pub` の private 化）である。各変更で `zig build && zig fmt --check src/ build.zig && zig build test --summary all` が通り、診断の増減が出ないことが完了条件で、削減が大きくても根拠のあるもの——`scripts/formal/` の Z3 モデル一式（PT1）、`src/fuzz_driver.zig` と `fuzz-driver` ステップ（PT2）、`bench_perf.py` の自前計測バックエンド（PT3）、`fix/builder.zig` の memcpy 系挿入（PT5）、`timezones.zig` の IANA 表・`cron.zig` の暦計算・`http_client` の `BoundedBody` と `SocketReaper`・YAML のアンカーとマージキー・bench スクリプトの分割——は残す判断が下りている |
...
| 順 | 対象 | 理由 |
|---|---|---|
| 1 | #438（GA11） | GA トラック第 3 段階の残り 1 件。GA7 が `cache-mode` を 2 能力へ解決する resolver を入れ、GA8 / GA9 / GA10 が setup-node の自動キャッシュ・checkout の credential・node20 の deprecated 化を同じ capability の形に載せたので、`concurrency.queue` と `cancel-in-progress` の矛盾検出（SYN025）も同じ枠に収まる。PR #519 がレビュー待ちである |
| 2 | #440 / #441（GA13 / GA14） | GA トラック第 4 段階の残り。`prefer-self-repository` と仕様追従用フィクスチャカタログで、追従漏れ自体を検出する側の整備である。2 件とも PR がレビュー待ちである（#440 = PR #521、#441 = PR #522） |
| 3 | #414〜#417（AF11〜AF14） | autofix 第 2 波の残り。`secrets: inherit` の展開と SC003 の `patched_version` bump は既存基盤で届くが、SEC002 の github-script `script:` と SC001 のコンテナ digest ピンは JS 書き換えとレジストリ I/O のため別 ADR を要する |
| 4 | #426（G38 / G39 の起票） | 実運用 CI の三者比較から出た gap のうち、parity doc への記録だけが残る PR である。対応する検出バグ #424 / #425 は PR #508 / #507 で閉じており、doc 側を追いつかせれば G1〜G39 が同じ地点に並ぶ |

v0.2.0 の GitHub Actions 仕様追従（#427 = GA0）は 14 本の子 issue を持ち、第 1 段階の P0 4 件（#428〜#431）と第 2 段階の `background` / `wait` / `parallel`（#432 / #433）は片付いている。
残る直列変更は無く、第 3 段階以降は capability モデルと追従漏れ検出の整備になる。
第 3 段階のうち GA7（#434）は PR #448 で着地し、`cache-mode` が `can_restore` / `can_save` の 2 能力へ解決され、SEC016 と PERF001 が同じ resolver を共有し、SEC024 が加わった。GA8（#435）/ GA9（#436）/ GA10（#437）も PR #517 / #523 / #518 で着地して action 側の capability が同じ形に載り、第 4 段階の GA12（#439）は PR #520 が SYN026 を足した。残るのは #438 と #440 / #441 で、いずれも PR #519 / #521 / #522 がレビュー待ちである。
第 4 段階は merge key の非対応検出・`prefer-self-repository`・仕様追従用フィクスチャカタログで、追従漏れ自体を検出する側の整備である。
過剰設計監査（#449 = PT0）は 45 本の子 issue がすべて閉じ、umbrella も close 済みである。振る舞いを変えない整理だけを対象にしたため、診断の増減は出ていない。
...
parity doc（`docs/design/external-linter-parity.md`）は main では G1〜G37 まで記録が追いつき、G38 / G39 は PR #426 が起票の形で待っている。
`bench/baseline.json` の数値と経緯の記録が同じ地点に並んでいる。

Phase 1〜4 はすべて閉じ、ルール追加の主戦場は #55 から bench（#262）へ移り、その bench も閉じた。
採点は全カテゴリを覆って recall 100%（117/117）・precision 100%・位置一致 97%、実行エラー 0 件で、
...
D7（#375）を PR #392 で閉じ、続く実運用 CI の三者比較が出した G36〜G39（#419 / #421 / #424 / #425）も PR #515 / #516 / #508 / #507 で 4 件とも閉じた。
