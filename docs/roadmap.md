...
オープンな PR / issue を main（`594a13d`）の実装状況と突き合わせ、以後の実施順序を示す。
...
| 順 | 対象 | 理由 |
|---|---|---|
| 1 | #421（G37。PR なし） | 唯一残るドッグフーディング由来のバグで、唯一の検出漏れを含む。`src/yaml/tokenizer.zig` の `scanPlainScalar` と `skipExpressionInterpolation` が plain scalar の行継続を読まないため、正当な式を EXPR001 にし、継続行側の汚染源を SEC002 が見落とす。FP と FN の両方が同じ欠落から出るので、直す価値が最も高い |
| 2 | #435〜#438（GA8〜GA11） | GA トラック第 3 段階の残り。GA7 が `cache-mode` を 2 能力へ解決する resolver を入れたので、setup-node の自動キャッシュ・checkout の credential・node20 の deprecated 化・concurrency の矛盾も同じ capability の形に載る |
...
| 4 | #439〜#441（GA12〜GA14） | GA トラック第 4 段階。merge key の非対応検出・`prefer-self-repository`・仕様追従用フィクスチャカタログで、追従漏れ自体を検出する側の整備である |
...
第 3 段階のうち GA7（#434）は PR #448 で着地し、`cache-mode` が `can_restore` / `can_save` の 2 能力へ解決され、SEC016 と PERF001 が同じ resolver を共有し、SEC024 が加わった。残る #435〜#438 は action 側の capability を同じ形で持つ。
