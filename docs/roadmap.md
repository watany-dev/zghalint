...
オープンな PR / issue を main（`57bde70`）の実装状況と突き合わせ、以後の実施順序を示す。
...
| `src/**/*.zig` | 56,962 行 |
| ユニットテスト | 1925 件（`zig build test` 緑）。#331 で低品質なテストの削除とテーブル駆動化を行った上で（PR #332）、SC007・ドッグフーディング・形式手法・autofix 拡張の分が積み増している |
...
| E2E テスト | `src/e2e_test.zig` が `tests/fixtures/e2e/*.yml`（62 本）と `tests/fixtures/e2e-action/*.yml`（8 本）の `# zghalint:expect RULE@line` / `forbid` コメントを読んで検証。兄弟ファイル `<name>.fixed` / `<name>.fixed-unsafe` を置いたフィクスチャは `--fix` / `--fix-unsafe` 適用後のバイト列も固定する（PR #328、現在 6 本） |
...
| 順 | 対象 | 理由 |
|---|---|---|
| 1 | #346 / #347 / #348（G22〜G24） | autofix 交差検証で出た fix 側の欠陥 3 本。#346 は `--fix` がタイポを特権トリガ `pull_request_target` へ直すもので、直した結果ワークフローが危険になる。誤って直す fix は指摘しない検出より害が大きく、他の全てに優先する |
| 2 | #307 の抽出器修復 | `scripts/formal/impl.py` が `security.zig` の表の改名に追随できず落ちている。F1〜F7 を埋めた結果が本当に反映されたかを確認する手段が今は無い。CI に載っていないことが腐敗を許した原因なので、修復と同時に疎通だけでも回す |
| 3 | `docs/rules.md` の見出し修正 | 表は 99 行で registry と同期済みなのに、見出しが「91 rules」のまま。#242 の同期テストは ID 集合しか見ないので本文の数字は守られない |
| 4 | #284 / #349 | ファイルが検査対象から落ちる系 2 本。#284 は中身のないワークフローを `InvalidValue` ではなく診断として返すもので bench に残る唯一の実行エラー、#349 は `*-dependabot.yml` を Dependabot 設定と誤認してワークフロー検査を飛ばすもの |
| 5 | #281 / #286 | 残る parity gap のルール追加 2 本。#281 は `needs:` の未知 job と循環、#286 は API トークンでの publish を trusted publishing へ誘導する |
...
検出そのものと並んで、指摘を直せるかどうか（#322 の autofix 拡張。AF1〜AF5 が全て着地し fix を持つルールは 99 中 49 になった）が主戦場になり、次の課題は fix の網羅ではなく正しさ（G22〜G24）へ移っている。ここに、
