...
オープンな PR / issue を main（`cfc9009`）の実装状況と突き合わせ、以後の実施順序を示す。
...
| `src/**/*.zig` | 57,430 行 |
| ユニットテスト | 1936 件（`zig build test` 緑）。#331 で低品質なテストの削除とテーブル駆動化を行った上で（PR #332）、SC007・ドッグフーディング・形式手法・autofix 拡張・fix の衝突修正の分が積み増している |
...
| E2E テスト | `src/e2e_test.zig` が `tests/fixtures/e2e/*.yml`（65 本）と `tests/fixtures/e2e-action/*.yml`（8 本）の `# zghalint:expect RULE@line` / `forbid` コメントを読んで検証。兄弟ファイル `<name>.fixed` / `<name>.fixed-unsafe` を置いたフィクスチャは `--fix` / `--fix-unsafe` 適用後のバイト列も固定する（PR #328、現在 11 本） |
...
| 3 | #281 / #286 | bench 由来の parity gap で残るルール追加 2 本。#281 は `needs:` の未知 job と循環、#286 は API トークンでの publish を trusted publishing へ誘導する |
...
実コーパスの FP（#305）は PR #317 で、ドッグフーディングの 5 本（#333〜#337）は PR #340〜#342 / #351 / #352 で全て片付いた。検出そのものの課題として残るのは、ファイルが丸ごと検査から落ちる #284（空ファイル。#349 は PR #357 で解消済み）とルール追加 2 本（#281 / #286）、そして実運用から新たに出た G26〜G28（#358〜#360）である。
