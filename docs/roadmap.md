...
| 順 | 対象 | 理由 |
|---|---|---|
| 1 | #410 / #411（AF7 / AF8） | autofix 第 2 波の P1 のうち安全側の 2 本。EXPR002〜EXPR004 の did-you-mean は AF1 の `rename` と EXPR006 の式バイトオフセットで、ACT001 の `shell:` 挿入は BP004 の `shell_insertion_byte` でそのまま閉じる。パーサ拡張が要らず、通常 lint の壁時間と RSS を動かさない |
| 2 | #412 / #413（AF9 / AF10） | 同じく P1 だが `--fix-unsafe` 側。SYN012 は SYN011 と同じマッピング要素削除、SEC014 は単純な `==` / `!=` に限った式の書き換えで足りる |
| 3 | #414 / #415（AF11 / AF12） | ローカル完結の SEC010 から、`--fix` 時に追加 GraphQL を引く SC003 へ。#415 は Advisory 型が `patched_version` を既に持つので、bump 先の決定に新しいデータ源が要らない |
