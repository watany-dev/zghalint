...
オープンな PR / issue を main（`15939b4`）の実装状況と突き合わせ、以後の実施順序を示す。
...
| 過剰設計監査 #449（PT0〜PT45 = #450〜#494） | `ponytail-audit` によるリポジトリ全体の監査から起票した umbrella。対象は死んだコード・std が既に持つ自前実装・実装が 1 つしかない抽象・誰も設定しない config・同じ論理をより短く書ける箇所に限り、正しさ・セキュリティ・性能は通常のレビューへ回す。45 本の子 issue が未使用サブシステム約 1,930 行（PT1 / PT2）、重複の集約約 600 行（PT5〜PT8 / PT11〜PT13 ほか）、std への置換約 250 行（PT10 / PT15〜PT18 / PT21 / PT26 ほか）、ツーリングと CI 約 390 行（PT3 / PT4 / PT9 / PT14 / PT19 / PT20 / PT45）を見込む。各変更で `zig build && zig fmt --check src/ build.zig && zig build test --summary all` が通り、診断の増減が出ないことが完了条件である。`timezones.zig` の IANA 表・`cron.zig` の暦計算・`http_client` の `BoundedBody` と `SocketReaper`・YAML のアンカーとマージキー・bench スクリプトの分割は、根拠を確認した上で残すと umbrella に記録されている |
...
過剰設計監査（#449 = PT0）の 45 件は振る舞いを変えない整理で、検出バグや GA トラックとは独立に進められる。
