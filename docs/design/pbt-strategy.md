...
入力クラスはシードだけでなくミューテーションでも決まる。ここまで足りなかったのは、
コーパスの別エントリの断片が入力の**途中**に入る形だった。`generate` の splice は
切った位置から後ろを全部置き換えるので、他のシードのジョブ本体が既存の `jobs:` の下に
着地することがない。行単位で他のシードから 1 行を接ぐミューテーションと、行を別の行境界へ
移すミューテーション (`steps:` が `runs-on:` の上に来る、シーケンス要素が親から離れる)
を足した。どちらも既存の 16 種の編集からは出てこない形。

この 2 種を足したキャンペーンが出したのは `FixBrokeParse`、つまり autofix が解析を
壊す形だった。`on: push\n  <: *c` の 2 行目はどのノードにもならないが、行としては
`on:` より深いのでエントリのもの。`on:` 行の末尾を挿入位置にすると、入れたばかりの
`concurrency:` ブロックがこの行を自分のエントリとして読み、見逃されていた未定義
エイリアスが解析エラーに変わった。入れ子の値の側は既に字下げされた末尾を範囲に
含めていたが、スカラー値の側は自分の改行で止まっていた。

なお長いキャンペーンは ReleaseFast でビルドしたドライバ (`zig build fuzz-driver
-Doptimize=ReleaseFast`) で回す。Debug の 6 倍ほど速いので、同じ時間で 6 倍の入力に
当たる。この形も 5 万回 42 秒で出た。

以降のキャンペーンが出したのは、**「どこまでが 1 つのトークン / エントリか」を
決める補助的な走査が、トークナイザと食い違う**という同じ形の 4 件である。
(a) フローコレクションの外の `}` と `${{` はただのスカラー文字であって構造ではない。
(b) ブロックスカラーは自分のヘッダ行と同じ字下げの行を content にしない。
(c) コメント行は字下げされた末尾を終わらせない (content でもキーでもない)。
(d) 本文の無いブロックスカラーはヘッダ行で終わる。いずれも範囲が伸びすぎるか
足りなさすぎるかで、挿入がスカラーの内側や次のキーの向こう側に落ちた。

続く 4 件は、挿入アンカーが**入力の持たないブロック配置を前提にする**という
これまでと同じ根だが、前提の外し方がそれぞれ違う。閉じないフローコレクションは
自分の行しか持てないので、同じ字下げで下に書かれたキーは本来のブロック
マッピングとして読み直す (そうしないと `{` が挿入で値の位置から押し出された
ときだけキーが現れ、`--fix` がファイルの持つキーを変えた)。`{uses: x}` と
書かれたステップには挿入できるブロック行が無いので own-line と数えない。
`uses:` の下にパーサが捨てた行が残るときは、値の末尾に書いた `with:` が
その行を自分のものとして拾う (`<: *g` が本物のマージキーになり、エイリアスの
アンカーが無くて解析が落ちた) ので、アンカーを持たせない。閉じた引用スカラーの
直後の文字は次のトークンの先頭であって、引用の続きではない (`"""""` を
「空スカラー 2 つと閉じない引用符」ではなく「1 つのスカラーと余りの引用符」と
読むと、SEC007 が `permissions:` 行を開いたままのスカラーの中に毎ラウンド
書き足した)。

「パーサが捨てた行」はもう一つの形も出した。前回は挿入がその行を拾う形だったが、
今度は**削除がその行を露出させる**。`on:\n workflow_call:\n    x:\n  *r ` の `*r` は
`x:` より浅いのでどのノードにもならないが、SYN011 が `x:` を消すとイベントに値が
なくなり、次のパスで `*r` がその値になる。未定義エイリアスで解析全体が落ちた。
イベントの最後のキーを消すときは削除範囲を出さない。**同じ「捨てた行」が、挿入と
削除の両方から到達される**という形である。

エスケープ列を入れたキャンペーンが出した 2 件も、既知の 2 つの形の残りだった。
1 件目、`services:` 配下の値がシーケンスだと解析全体が `InvalidValue` で落ちる。
SYN001 のリネームが `erices:` を `services:` に直した先で当たる、`credentials:` と
同じ形。SYN004 の型不一致として報告して続ける。2 件目は、ブロックスカラーの
トークンが**自分を閉じる行の先頭で終わる**ことの帰結。その行のキーとの間に newline
トークンが無いので、ゴミ行を読み飛ばす処理がキーごと飲み込んでいた。上に 1 行
挿入するとスカラーが早く終わってキーが現れ、`--fix` がファイルの持つキーを変える。
トークンが既に次の行から始まっているなら、それは「この行の残り」ではない。

変異器にはさらに 2 つ足した。行の値をブロックスカラーのヘッダ + 本文に
書き換える操作と、ファイル全体を CRLF 化する操作である。辞書にあるのは
ヘッダだけで、スカラーの終端を決めるのはキーより深い本文のほうだが、
ヘッダを無作為に差し込んでも本文が付くことはほぼ無い。CR も同じく、行中に
`"\r\n"` を差し込むだけではスパンやスカラー値を通る経路に届かない。3 つ目は行の値を
二重引用スカラーに書き換えてエスケープ列を入れる操作である。デコード後の値は元の
トークンより短いので、値の中のオフセットがソースのオフセットにならない
(`DiagnosticSpanOutOfRange` を出したエイリアスと同じ形)。`\n` を無作為に差し込むと
引用符の外に落ち、ただの 2 文字にしかならない。

4 つ目は、ある行の値にアンカーを置き、別の行の値をそのエイリアスに書き換える操作
である。辞書には `&anchor` と `*anchor` があるが、無作為な 2 回の挿入が同じ名前を
指すことはまず無く、解決済みのエイリアスを通る経路に入れていなかった。この 1 つで
60 万回のキャンペーンが 28 件出し、2 つの形に収束した。どちらも「エイリアスの
ノードは定義側の値を指す」ことの帰結である。

1 つ目、削除の fix がアンカーの定義ごと消す。SYN011 がイベントの受け付けない
フィルタキーを消した先に `&b` が書かれていて、下の `*b` が解決しなくなり、次の解析が
診断を 1 件も出せなくなった。ファイルを解析可能なまま保てない fix は、診断を残す
より価値が低い。削除範囲の中に、範囲の外から参照されているアンカーがあれば、その
edit は捨てる。`run:` の中の `&&` や `*` はアンカーではないので、`&` / `*` の直前が
YAML がマーカーを読む位置かどうかで判定する。

2 つ目、複数 edit を持つ fix の一部だけが当たる。`run: *c` の値はエイリアスの指す
`${{inputs.t}}` なので SEC002 は env 束縛の fix を作るが、`run:` 本文を書き換える
edit のスパンはエイリアスの使用位置ではなく定義位置を指す。`expects` が合わずに
その edit だけが落ち、`env:` を挿入する edit は当たる。参照されない束縛が毎回 1 つ
増えて収束しない。#223 が「fix は丸ごと適用するか、まったく適用しないか」と決めた
のと同じ理由が、重なりだけでなく不正な edit にも当てはまる。ソースと合わない edit
を 1 つでも持つ fix は丸ごと捨てる。再実行しても同じソースを読んで同じように捨てる
ので、skip 件数には数えない。

変異 5 (マージキー) を足した回 (60 万件) は 4 件、クラスは 2 つ。

1 つ目は `on:` の下で落ちた行に挿入アンカーを置いていた件。`on: &c` の下の `>`
が閉じないままファイル末尾まで飲み込んでいると、`on` の `full_span` は飲まれた
行の手前で終わるのに実体はその先まで伸びる。その `full_span.end_byte` に
`permissions:` を挿入するとブロックスカラーが途中で切れ、飲まれていた `<: *b`
が未定義エイリアスとして表に出て次のパースが落ちる。落ちた行 (`has_indented_tail`)
を持つ `on:` は挿入アンカーを提供しない。

2 つ目は `steps:` がマージキー経由でマッピングを持つ件。`steps:` の下の
`<<: *a` が job の `permissions:` を折り込むので `steps:` は空でないマッピングに
なり、`parseSteps` が `InvalidValue` を返してファイル全体のパースが失敗していた。
`services:` / `credentials:` と同じ扱いにする。型が違うセクション 1 つで他の
診断を全部落とすのは割に合わないので、SYN004 として報告して先へ進む。
