...
#### G20 (#304). ネットワークに到達できないとき SC003〜SC006 が黙って沈黙する — 要 UX 改善
...
#### G21 (#305). 実行時に作られるローカル action パスに DEP004 が誤検出を出す — 要ルール改善
...
- [ ] G20 (#304): ネットワーク取得に失敗したルールを黙って落とさず注記する
- [ ] G21 (#305): DEP004 から `actions/checkout` の `path:` が作るディレクトリを除外する
