...
リリースジョブがどちらも満たせない。リリース / デプロイのジョブでキャッシュを
どうするかは SEC016 の担当なので、PERF001 は SEC016 と同じスコープ判定
(`isCachePoisoningScope`) のジョブから手を引くようにした。PERF001 は
ワークフロー単位の検査になっている (トリガを見るため)。
