...
### 4.10 2026-09-10 の rc1 / rc2 / 現行比較

`v0.0.1-rc.1`・`v0.0.1-rc.2`・現行 `main` (`94fd1c9`) を、既存の採点と
`--perf` の 3 シナリオで同じマシン・同じファイル集合に流した。
`v0.0.1-rc.2` は `d43a3f4` (Zig 0.16)。現行はその 105 コミット先で、
`--version` 文字列はまだ `v0.0.1-rc.2` のまま。

環境は Linux x86_64 / 4 logical CPU、hyperfine 1.18.0 (10 runs / warmup 3)、
GNU time。ケースとコーパスは現行 `main` のもの (cases 138 ファイル /
2,363 行、huge 10,035 行、many-small は 228 件を 1,000 ファイルに敷き詰め
89,607 行)。rival は入れず zghalint だけを測った。`network` は
`GITHUB_TOKEN` 未設定のため未計測。

| バイナリ | 出所 | `--version` | コミット |
|---|---|---|---|
| rc1 | GitHub Release `linux-x86_64` | `v0.0.1-rc.1` | `7600004` |
| rc2 | GitHub Release `linux-x86_64` | `v0.0.1-rc.2` | `d43a3f4` |
| 現行 | `zig build -Doptimize=ReleaseFast` | `v0.0.1-rc.2` (zon 未バンプ) | `94fd1c9` |

順序バイアスを除くため rc1→rc2→現行と現行→rc2→rc1 の 2 パスを取り、
下表は 2 パスの平均。各パスの σ は cases / huge で 0.1〜0.3 ms、
many-small で 0.4〜2.3 ms (rc1 の 2 パス目だけ 7.1 ms)。

#### 性能 (`scripts/bench.py --perf`)

| シナリオ | rc1 | rc2 | 現行 `94fd1c9` |
|---|---|---|---|
| cases (138 ファイル / 2,363 行) | 3.2 ms · 1.9 MiB | 3.5 ms · 2.1 MiB | 4.0 ms · 2.2 MiB |
| huge (1 ファイル / 10,035 行) | 9.9 ms · 8.0 MiB | 11.3 ms · 8.3 MiB | 15.3 ms · 8.9 MiB |
| many-small (1,000 ファイル / 89,607 行) | 86.7 ms · 7.7 MiB | 92.5 ms · 7.7 MiB | 128.3 ms · 8.4 MiB |
...
actionlint / zizmor は未導入なので zghalint 列だけを見る。unique-win は
他ツールが居ないため検出数そのものになり、版比較には使わない。

| 版 | recall | precision | 位置一致 | FN | FP |
|---|---|---|---|---|---|
| rc1 | 96% (111/116) | 92% | 92% (107/116) | 5 | 10 |
| rc2 | 100% (116/116) | 100% | 97% (112/116) | 0 | 0 |
| 現行 | 100% (116/116) | 100% | 97% (112/116) | 0 | 0 |

rc1 の FN は rc2 までに入ったルール (EXPR011 のオブジェクト軸、SYN021 /
SYN022、SEC016 の tag push と setup-uv 既定キャッシュ)。FP は同じく
rc2 までに沈黙させたケース (動的マトリクス、値位置の `||` / `&&`、
`$` 自己参照、`ubuntu-slim` ほか)。rc2 と現行は現行ケース上では同じ
スコアで、ファズ修正は既存 expect の検出集合を動かしていない。
