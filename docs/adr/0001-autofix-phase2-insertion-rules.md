...
- 追記（#223）: 形式検証で「1 つの Fix の編集が部分適用されうる」ことが分かったため、engine 側は Fix 単位の原子性（衝突した Fix は全体を捨てる）と、登録順を明示した tie-break を持つようになった。本 D5 が扱うゼロ幅の同 byte 挿入は衝突ではないため、両方が残る挙動は変わらない
