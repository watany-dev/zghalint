...
def test_script_injection_always_detected(zghalint_bin, content):
    ...
    ids = _rule_ids(data)
    assert "SEC002" in ids, f"SEC002 not detected.\nWorkflow:\n{content}\nDetected rules: {ids}"
...
def test_unpinned_action_always_detected(zghalint_bin, content):
    ...
    data = lint_workflow_json(zghalint_bin, content)
    ids = _rule_ids(data)
    assert "SEC001" in ids, f"SEC001 not detected.\nWorkflow:\n{content}\nDetected rules: {ids}"
...
def test_hardcoded_secret_always_detected(zghalint_bin, content):
    ...
    data = lint_workflow_json(zghalint_bin, content)
    ids = _rule_ids(data)
    assert "SEC003" in ids, f"SEC003 not detected.\nWorkflow:\n{content}\nDetected rules: {ids}"
...
def test_script_injection_severity_is_error(zghalint_bin, content):
    ...
    for d in sec002:
        assert d["severity"] == "error", f"SEC002 severity is {d['severity']}, expected error"
...
def test_hardcoded_secret_severity_is_error(zghalint_bin, content):
    ...
    for d in sec003:
        assert d["severity"] == "error", f"SEC003 severity is {d['severity']}, expected error"
